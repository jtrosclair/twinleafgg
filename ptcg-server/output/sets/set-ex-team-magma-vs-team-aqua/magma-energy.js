...
class MagmaEnergy extends energy_card_1.EnergyCard {
    constructor() {
        super(...arguments);
        ...
        this.setNumber = '87';
        this.name = 'Magma Energy';
        this.fullName = 'Magma Energy MA';
        this.text = 'Magma Energy can be attached only to a Pokémon with Team Magma in its name. Magma Energy provides [F] and/or [D] Energy but provides 2 Energy at a time. (Doesn\'t count as a basic Energy card when not in play and has no effect other than providing Energy.) At the end of your turn, discard Magma Energy.';
    }
    ...
    getEnergyToProvide(attackCost, existingEnergy) {
        const needsFighting = attackCost.includes(card_types_1.CardType.FIGHTING);
        ...
        const fightingCount = this.countEnergyType(existingEnergy, card_types_1.CardType.FIGHTING);
        const darkCount = this.countEnergyType(existingEnergy, card_types_1.CardType.DARK);
        const requiredFighting = attackCost.filter(c => c === card_types_1.CardType.FIGHTING).length;
        ...
        if (needsFighting && needsDark) {
            if (!hasEnoughFighting && !hasEnoughDark) {
                return [card_types_1.CardType.FIGHTING, card_types_1.CardType.DARK];
            }
            if (!hasEnoughFighting) {
                return [card_types_1.CardType.FIGHTING, card_types_1.CardType.FIGHTING];
            }
    ...
    reduceEffect(store, state, effect) {
        ...
        if (effect instanceof play_card_effects_1.AttachEnergyEffect) {
            if (effect.energyCard === this && !((_a = effect.target.getPokemonCard()) === null || _a === void 0 ? void 0 : _a.tags.includes(card_types_1.CardTag.TEAM_MAGMA))) {
        ...
        if (effect instanceof check_effects_1.CheckTableStateEffect) {
            state.players.forEach(player => {
                player.forEachPokemon(game_1.PlayerType.BOTTOM_PLAYER, cardList => {
                    ...
                    if (pokemonCard && !pokemonCard.tags.includes(card_types_1.CardTag.TEAM_MAGMA)) {
...
exports.MagmaEnergy = MagmaEnergy;
