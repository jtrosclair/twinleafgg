...
function* playCard(next, store, state, effect, self) {
    ...
    player.deck.cards.forEach((card, index) => {
        ...
        if ((card instanceof game_1.PokemonCard && card.stage === card_types_1.Stage.BASIC && card.tags.includes(card_types_1.CardTag.TEAM_MAGMA)) ||
            (card.superType === card_types_1.SuperType.ENERGY && card.energyType === card_types_1.EnergyType.BASIC && card.name === 'Fighting Energy')) {
            /**/
        }
        else {
            blocked.push(index);
    ...
    cards.forEach((card, index) => {
        (0, prefabs_1.MOVE_CARDS)(store, state, player.deck, player.hand, { cards: [card], sourceCard: self });
    });
    cards.forEach((card, index) => {
