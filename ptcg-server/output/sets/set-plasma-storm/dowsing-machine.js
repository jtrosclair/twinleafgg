...
function* playCard(next, store, state, self, effect) {
    ...
    let recovered = [];
    yield store.prompt(state, new choose_cards_prompt_1.ChooseCardsPrompt(player, game_message_1.GameMessage.CHOOSE_CARD_TO_HAND, player.discard, { superType: card_types_1.SuperType.TRAINER }, { min: 1, max: 1, allowCancel: true }), selected => {
        recovered = selected || [];
        next();
    });
