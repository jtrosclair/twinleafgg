            ...
            return store.prompt(state, new game_1.ChoosePokemonPrompt(player.id, game_1.GameMessage.CHOOSE_POKEMON_TO_DAMAGE, game_1.PlayerType.TOP_PLAYER, [game_1.SlotType.ACTIVE, game_1.SlotType.BENCH], { allowCancel: false, blocked }), targets => {
                ...
                if (target.tools.length > 0) {
                    ...
                    if (target.tools.length > 1) {
                        return store.prompt(state, new game_1.ChooseCardsPrompt(player, game_1.GameMessage.CHOOSE_CARD_TO_DISCARD, target, { superType: game_1.SuperType.TRAINER, trainerType: game_1.TrainerType.TOOL }, { min: 1, max: 1, allowCancel: false }), selected => {
