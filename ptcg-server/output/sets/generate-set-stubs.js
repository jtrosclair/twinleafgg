...
function getTags(subtypes) {
    ...
    for (const st of subtypes) {
        switch (st) {
            case 'EX':
                tags.push('CardTag.POKEMON_EX');
                break;
            case 'ex':
                tags.push('CardTag.POKEMON_ex');
                break;
            case 'V':
                tags.push('CardTag.POKEMON_V');
                break;
            case 'VMAX':
                tags.push('CardTag.POKEMON_VMAX');
                break;
            case 'VSTAR':
                tags.push('CardTag.POKEMON_VSTAR');
                break;
            case 'GX':
                tags.push('CardTag.POKEMON_GX');
                break;
            case 'TAG TEAM':
                tags.push('CardTag.TAG_TEAM');
                break;
            case 'MEGA':
                tags.push('CardTag.MEGA');
                break;
            case 'BREAK':
                tags.push('CardTag.BREAK');
                break;
            case 'V-UNION':
                tags.push('CardTag.POKEMON_VUNION');
                break;
            case 'Prism Star':
                tags.push('CardTag.PRISM_STAR');
                break;
            case 'Ultra Beast':
                tags.push('CardTag.ULTRA_BEAST');
                break;
            case 'Team Plasma':
                tags.push('CardTag.TEAM_PLASMA');
                break;
            case 'Single Strike':
                tags.push('CardTag.SINGLE_STRIKE');
                break;
            case 'Rapid Strike':
                tags.push('CardTag.RAPID_STRIKE');
                break;
            case 'Fusion Strike':
                tags.push('CardTag.FUSION_STRIKE');
                break;
        }
    }
    return tags;
}
function getTrainerType(subtypes) {
...
function getUseWhenInPlay(type) {
    switch (type) {
...
function filterGenericRules(rules) {
    return rules.filter(r => {
        const trimmed = r.trim();
        if (trimmed.startsWith('This stadium stays in play') || trimmed.startsWith('This Stadium stays in play'))
            return false;
        if (trimmed.startsWith('This card stays in play when you play it'))
            return false;
...
function convertEnergyNames(text) {
    if (!text)
        return text;
    const ENERGY_NAME_TO_SYMBOL = {
...
function parseDamage(dmgStr) {
    ...
    const mod = match[2];
    if (mod === '+')
        return { damage, damageCalculation: '+' };
    if (mod === '×' || mod === 'x')
        return { damage, damageCalculation: 'x' };
    if (mod === '-')
        return { damage, damageCalculation: '-' };
    return { damage };
}
...
// ==================== CARD CLASSIFICATION ====================
function cardNeedsReduceEffect(card) {
    ...
    if (card.attacks) {
        for (const attack of card.attacks) {
            if (attack.text && attack.text.trim() !== '')
                return true;
        }
    }
    return false;
}
function cardsAreIdentical(a, b) {
    if (a.name !== b.name)
        return false;
    const aAttacks = a.attacks || [];
    const bAttacks = b.attacks || [];
    if (aAttacks.length !== bAttacks.length)
        return false;
    for (let i = 0; i < aAttacks.length; i++) {
        if (aAttacks[i].name !== bAttacks[i].name)
            return false;
        if ((aAttacks[i].text || '') !== (bAttacks[i].text || ''))
            return false;
    }
    const aAbilities = a.abilities || [];
    const bAbilities = b.abilities || [];
    if (aAbilities.length !== bAbilities.length)
        return false;
    for (let i = 0; i < aAbilities.length; i++) {
        if (aAbilities[i].name !== bAbilities[i].name)
            return false;
        if ((aAbilities[i].text || '') !== (bAbilities[i].text || ''))
            return false;
    }
    return true;
}
function buildCrossSetIndex(setsDir, currentSetDir) {
    const index = new Map();
    let setDirs;
    try {
        setDirs = fs.readdirSync(setsDir).filter((d) => {
    ...
    for (const setDir of setDirs) {
        ...
        let files;
        try {
            files = fs.readdirSync(setPath).filter((f) => f.endsWith('.ts') && f !== 'index.ts' && f !== 'other-prints.ts');
        }
        catch (_b) {
            continue;
        }
...
// ==================== STUB GENERATION ====================
function generatePokemonStub(card, className, setCode, needsReduceEffect, fullName) {
    var _a;
    const lines = [];
    lines.push(STUB_HEADER);
    const tags = getTags(card.subtypes);
    const hasAbilities = card.abilities && card.abilities.length > 0;
    const hasAttacksWithText = (_a = card.attacks) === null || _a === void 0 ? void 0 : _a.some((a) => a.text && a.text.trim() !== '');
    // Imports
    const cardTypeImports = ['Stage', 'CardType'];
    if (tags.length)
        cardTypeImports.push('CardTag');
    ...
    if (needsReduceEffect) {
        ...
        const prefabs = [];
        if (hasAttacksWithText)
            prefabs.push('WAS_ATTACK_USED');
        if (hasAbilities)
            prefabs.push('WAS_POWER_USED');
        if (prefabs.length) {
    ...
    if (card.attacks && card.attacks.length > 0) {
        ...
        const attackStrs = card.attacks.map((a) => {
            const cost = (a.cost || []).filter((c) => c !== 'Free').map((c) => typeShort(c));
...
function generateTrainerStub(card, className, setCode, fullName) {
    const lines = [];
    lines.push(STUB_HEADER);
    const tags = getTags(card.subtypes);
...
function generateEnergyStub(card, className, setCode, fullName) {
    const lines = [];
    lines.push(STUB_HEADER);
...
function generateOtherPrintsEntry(entry) {
...
// ==================== INDEX GENERATION ====================
function generateIndex(setDir, setDirName, allCardFiles, otherPrintClasses) {
...
function main() {
    ...
    const cards = JSON.parse(fs.readFileSync(cardDataPath, 'utf-8'));
    if (!cards.length) {
        console.error('No cards found in card-data.json');
        process.exit(1);
    }
    const setCode = ((_a = cards[0].set) === null || _a === void 0 ? void 0 : _a.ptcgoCode) || '';
    if (!setCode) {
    ...
    const existingOtherPrintClasses = [];
    const existingOtherPrintSetNumbers = new Set();
    ...
    const existingClassNames = new Map(); // fileName → className
        ...
        for (const f of fs.readdirSync(setDir)) {
            if (f.endsWith('.ts') && f !== 'index.ts' && f !== 'other-prints.ts') {
                ...
                try {
                    ...
                    if (match)
                        existingClassNames.set(baseName, match[1]);
                }
                catch ( /* skip */_c) { /* skip */ }
            }
        }
    ...
    for (const card of sortedCards) {
        ...
        const fullName = count > 1
            ...
            : `${baseName} ${setCode}`;
        const category = card.supertype === 'Pokémon' ? 'pokemon' :
    ...
    for (let i = 0; i < cardActions.length; i++) {
        ...
        const card = ca.card;
        // Skip basic energies
        if (card.supertype === 'Energy' && ((_b = card.subtypes) === null || _b === void 0 ? void 0 : _b.includes('Basic'))) {
            ca.action = 'skip';
            continue;
        }
        ...
        for (const earlierIdx of sameNameIndices) {
            ...
            if (earlierCA.action === 'skip' && !existingFiles.has(earlierCA.fileName))
                continue;
        ...
        if (crossEntries) {
            const cardAttackNames = (card.attacks || []).map((a) => a.name);
            const cardAbilityNames = (card.abilities || []).map((a) => a.name);
            const source = crossEntries.find(e => {
                // Compare attack names
                if (e.attackNames.length !== cardAttackNames.length)
                    return false;
                for (let j = 0; j < cardAttackNames.length; j++) {
                    if (e.attackNames[j] !== cardAttackNames[j])
                        return false;
                }
                // Compare ability names
                if (e.abilityNames.length !== cardAbilityNames.length)
                    return false;
                for (let j = 0; j < cardAbilityNames.length; j++) {
                    if (e.abilityNames[j] !== cardAbilityNames[j])
                        return false;
                }
                return true;
            });
    ...
    for (const [fileName, className] of existingClassNames) {
        ...
        try {
            ...
            if (content.match(/extends\s+TrainerCard/))
                cat = 'trainer';
            else if (content.match(/extends\s+EnergyCard/))
                cat = 'energy';
        }
        catch ( /* skip */_d) { /* skip */ }
    ...
    if (newOtherPrints.length > 0) {
        ...
        for (const entry of newOtherPrints) {
            ...
            if (existingPath && existingPath !== entry.importPath) {
                ...
                const suffix = setDirPart ? setDirPart[1].split('-').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join('') : entry.setNumber;
    ...
    if (energyCount > 0)
        console.log(`  ${energyCount} Special Energy cards (TODO)`);
