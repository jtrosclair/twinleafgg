"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.setChaosRising = void 0;
const ampharos_1 = require("./ampharos");
const avalugg_1 = require("./avalugg");
const azs_tranquility_1 = require("./azs-tranquility");
const baltoy_1 = require("./baltoy");
const beedrill_ex_1 = require("./beedrill-ex");
const beldum_1 = require("./beldum");
const bergmite_1 = require("./bergmite");
const big_catch_net_1 = require("./big-catch-net");
const braixen_1 = require("./braixen");
const bubble_water_energy_1 = require("./bubble-water-energy");
const carnivine_1 = require("./carnivine");
const chespin_1 = require("./chespin");
const chesnaught_1 = require("./chesnaught");
const claydol_1 = require("./claydol");
const cobalion_ex_1 = require("./cobalion-ex");
const crobat_1 = require("./crobat");
const deoxys_1 = require("./deoxys");
const deoxys_2_1 = require("./deoxys-2");
const deoxys_3_1 = require("./deoxys-3");
const deoxys_4_1 = require("./deoxys-4");
const delibird_1 = require("./delibird");
const delphox_1 = require("./delphox");
const donphan_1 = require("./donphan");
const emolga_1 = require("./emolga");
const espurr_1 = require("./espurr");
const fennekin_1 = require("./fennekin");
const ferroseed_1 = require("./ferroseed");
const ferrothorn_1 = require("./ferrothorn");
const book_of_transformation_1 = require("./book-of-transformation");
const cinccino_ex_1 = require("./cinccino-ex");
const emma_1 = require("./emma");
const flaaffy_1 = require("./flaaffy");
const froakie_1 = require("./froakie");
const frogadier_1 = require("./frogadier");
const garbodor_1 = require("./garbodor");
const goodra_1 = require("./goodra");
const golbat_1 = require("./golbat");
const golisopod_1 = require("./golisopod");
const goomy_1 = require("./goomy");
const gourgeist_ex_1 = require("./gourgeist-ex");
const ho_oh_1 = require("./ho-oh");
const hyperrogue_ange_floette_1 = require("./hyperrogue-ange-floette");
const kakuna_1 = require("./kakuna");
const keldeo_1 = require("./keldeo");
const litleo_1 = require("./litleo");
const magnet_metal_energy_1 = require("./magnet-metal-energy");
const mareep_1 = require("./mareep");
const mega_floette_ex_1 = require("./mega-floette-ex");
const mega_greninja_ex_1 = require("./mega-greninja-ex");
const mega_pyroar_ex_1 = require("./mega-pyroar-ex");
const mega_dragalge_ex_1 = require("./mega-dragalge-ex");
const metagross_1 = require("./metagross");
const metang_1 = require("./metang");
const meowstic_1 = require("./meowstic");
const minccino_1 = require("./minccino");
const nitro_fire_energy_1 = require("./nitro-fire-energy");
const ninetales_1 = require("./ninetales");
const octillery_1 = require("./octillery");
const phanpy_1 = require("./phanpy");
const phantump_1 = require("./phantump");
const patrat_1 = require("./patrat");
const philippe_1 = require("./philippe");
const roxies_performance_1 = require("./roxies-performance");
const prism_tower_1 = require("./prism-tower");
const pumpkaboo_1 = require("./pumpkaboo");
const quilladin_1 = require("./quilladin");
const qwilfish_1 = require("./qwilfish");
const remoraid_1 = require("./remoraid");
const sliggoo_1 = require("./sliggoo");
const skrelp_1 = require("./skrelp");
const special_red_card_1 = require("./special-red-card");
const skuntank_1 = require("./skuntank");
const stunky_1 = require("./stunky");
const sudowoodo_1 = require("./sudowoodo");
const tauros_1 = require("./tauros");
const trevenant_1 = require("./trevenant");
const trubbish_1 = require("./trubbish");
const vulpix_1 = require("./vulpix");
const watchog_1 = require("./watchog");
const weedle_1 = require("./weedle");
const wimpod_1 = require("./wimpod");
const xerneas_1 = require("./xerneas");
const zubat_1 = require("./zubat");
const other_prints_1 = require("./other-prints");
exports.setChaosRising = [
    new weedle_1.Weedle(),
    new kakuna_1.Kakuna(),
    new beedrill_ex_1.Beedrillex(),
    new carnivine_1.Carnivine(),
    new chespin_1.Chespin(),
    new quilladin_1.Quilladin(),
    new chesnaught_1.Chesnaught(),
    new vulpix_1.Vulpix(),
    new ninetales_1.Ninetales(),
    new ho_oh_1.HoOh(),
    new fennekin_1.Fennekin(),
    new braixen_1.Braixen(),
    new delphox_1.Delphox(),
    new litleo_1.Litleo(),
    new mega_pyroar_ex_1.MegaPyroarex(),
    new remoraid_1.Remoraid(),
    new octillery_1.Octillery(),
    new delibird_1.Delibird(),
    new keldeo_1.Keldeo(),
    new froakie_1.Froakie(),
    new frogadier_1.Frogadier(),
    new mega_greninja_ex_1.MegaGreninjaex(),
    new bergmite_1.Bergmite(),
    new avalugg_1.Avalugg(),
    new wimpod_1.Wimpod(),
    new golisopod_1.Golisopod(),
    new mareep_1.Mareep(),
    new flaaffy_1.Flaaffy(),
    new ampharos_1.Ampharos(),
    new emolga_1.Emolga(),
    new deoxys_1.Deoxys(),
    new deoxys_2_1.Deoxys2(),
    new deoxys_3_1.Deoxys3(),
    new deoxys_4_1.Deoxys4(),
    new mega_floette_ex_1.MegaFloetteex(),
    new espurr_1.Espurr(),
    new meowstic_1.Meowstic(),
    new phantump_1.Phantump(),
    new trevenant_1.Trevenant(),
    new pumpkaboo_1.Pumpkaboo(),
    new gourgeist_ex_1.Gourgeistex(),
    new xerneas_1.Xerneas(),
    new sudowoodo_1.Sudowoodo(),
    new phanpy_1.Phanpy(),
    new donphan_1.Donphan(),
    new baltoy_1.Baltoy(),
    new claydol_1.Claydol(),
    new zubat_1.Zubat(),
    new golbat_1.Golbat(),
    new crobat_1.Crobat(),
    new qwilfish_1.Qwilfish(),
    new stunky_1.Stunky(),
    new skuntank_1.Skuntank(),
    new trubbish_1.Trubbish(),
    new garbodor_1.Garbodor(),
    new skrelp_1.Skrelp(),
    new beldum_1.Beldum(),
    new metang_1.Metang(),
    new metagross_1.Metagross(),
    new ferroseed_1.Ferroseed(),
    new ferrothorn_1.Ferrothorn(),
    new cobalion_ex_1.Cobalionex(),
    new mega_dragalge_ex_1.MegaDragalgeex(),
    new goomy_1.Goomy(),
    new sliggoo_1.Sliggoo(),
    new goodra_1.Goodra(),
    new tauros_1.Tauros(),
    new patrat_1.Patrat(),
    new watchog_1.Watchog(),
    new minccino_1.Minccino(),
    new cinccino_ex_1.Cinccinoex(),
    new special_red_card_1.SpecialRedCard(),
    new big_catch_net_1.BigCatchNet(),
    new book_of_transformation_1.BookOfTransformation(),
    new azs_tranquility_1.AzsTranquility(),
    new philippe_1.Philippe(),
    new roxies_performance_1.RoxiesPerformance(),
    new emma_1.Emma(),
    new hyperrogue_ange_floette_1.HyperrogueAngeFloette(),
    new prism_tower_1.PrismTower(),
    new nitro_fire_energy_1.NitroFireEnergy(),
    new magnet_metal_energy_1.MagnetMetalEnergy(),
    new bubble_water_energy_1.BubbleWaterEnergy(),
    // Other prints
    new other_prints_1.TransformationTomeCRI(), // CRI 83
];
