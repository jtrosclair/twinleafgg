"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.setGenerations = void 0;
const other_prints_1 = require("./other-prints");
const basic_energies_1 = require("./basic-energies");
const charmeleon_1 = require("./charmeleon");
const jolteon_ex_1 = require("./jolteon-ex");
const max_revive_1 = require("./max-revive");
const meowstic_ex_1 = require("./meowstic-ex");
const red_card_1 = require("./red-card");
const revitalizer_1 = require("./revitalizer");
const team_flare_grunt_1 = require("./team-flare-grunt");
exports.setGenerations = [
    new jolteon_ex_1.JolteonEX(),
    new max_revive_1.MaxRevive(),
    new meowstic_ex_1.MeowsticEX(),
    new red_card_1.RedCard(),
    new revitalizer_1.Revitalizer(),
    new team_flare_grunt_1.TeamFlareGrunt(),
    // FA/Radiant Collection
    new charmeleon_1.Charmeleon(),
    // Basic energies
    new basic_energies_1.GrassEnergy(),
    new basic_energies_1.FireEnergy(),
    new basic_energies_1.WaterEnergy(),
    new basic_energies_1.LightningEnergy(),
    new basic_energies_1.PsychicEnergy(),
    new basic_energies_1.FightingEnergy(),
    new basic_energies_1.DarknessEnergy(),
    new basic_energies_1.MetalEnergy(),
    new basic_energies_1.FairyEnergy(),
    new other_prints_1.PonytaGEN(),
    new other_prints_1.RapidashGEN(),
    new other_prints_1.PikachuGEN(),
    new other_prints_1.RaichuGEN(),
    new other_prints_1.GolbatGEN(),
    new other_prints_1.SlowpokeGEN(),
    new other_prints_1.HaunterGEN(),
    new other_prints_1.GengarGEN(),
    new other_prints_1.MrMimeGEN(),
    new other_prints_1.CrushingHammerGEN(),
    new other_prints_1.EnergySwitchPKGEN(),
    new other_prints_1.EvosodaGEN(),
    new other_prints_1.MaintenanceGEN(),
    new other_prints_1.PokeBallGEN(),
    new other_prints_1.PokemonFanClubGEN(),
    new other_prints_1.ShaunaGEN(),
    new other_prints_1.DoubleColorlessEnergyGEN(),
    new other_prints_1.JolteonEX2GEN(),
    new other_prints_1.TeamFlareGrunt2GEN(),
    new other_prints_1.Raichu2GEN(),
    new other_prints_1.WobbuffetGEN(),
    new other_prints_1.YveltalGEN(),
    new other_prints_1.SwirlixGEN(),
    new other_prints_1.WallyGEN(),
    new other_prints_1.Pikachu2GEN(),
    new other_prints_1.GardevoirExGEN(),
    new other_prints_1.MGardevoirExGEN(),
];
