...
class Amarys extends trainer_card_1.TrainerCard {
    ...
    reduceEffect(store, state, effect) {
        if (effect instanceof play_card_effects_1.TrainerEffect && effect.trainerCard === this) {
            ...
            (0, prefabs_1.ADD_MARKER)(this.AMARYS_USED_MARKER, effect.player, this);
        }
        if (effect instanceof game_phase_effects_1.EndTurnEffect && (0, prefabs_1.HAS_MARKER)(this.AMARYS_USED_MARKER, effect.player, this)) {
            const hand = effect.player.hand;
            const discard = effect.player.discard;
            if (hand.cards.length >= 5)
                hand.moveCardsTo(hand.cards, discard);
        }
        (0, prefabs_1.REMOVE_MARKER_AT_END_OF_TURN)(effect, this.AMARYS_USED_MARKER, this);
        return state;
    }
}
