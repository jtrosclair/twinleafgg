...
class DarkClaw extends trainer_card_1.TrainerCard {
    ...
    reduceEffect(store, state, effect) {
        if (effect instanceof attack_effects_1.DealDamageEffect && effect.source.tools.includes(this)) {
            ...
            const checkPokemonTypeEffect = new check_effects_1.CheckPokemonTypeEffect(sourcePokemon);
            store.reduceEffect(state, checkPokemonTypeEffect);
            if (checkPokemonTypeEffect.cardTypes.includes(card_types_1.CardType.DARK) && effect.damage > 0) {
                effect.damage += 20;
            }
        }
        return state;
