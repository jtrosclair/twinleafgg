"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.setNobleVictories = void 0;
const other_prints_1 = require("./other-prints");
const archen_1 = require("./archen");
const archeops_1 = require("./archeops");
const blitzle_1 = require("./blitzle");
const chandelure_1 = require("./chandelure");
const durant_1 = require("./durant");
const eelektrik_1 = require("./eelektrik");
const eviolite_1 = require("./eviolite");
const kyurem_1 = require("./kyurem");
const lampent_1 = require("./lampent");
const palpitoad_1 = require("./palpitoad");
const petilil_1 = require("./petilil");
const plume_fossil_1 = require("./plume-fossil");
const seismitoad_1 = require("./seismitoad");
const terrakion_1 = require("./terrakion");
const trubbish_1 = require("./trubbish");
const tympole_1 = require("./tympole");
const tynamo_1 = require("./tynamo");
const tynamo_2_1 = require("./tynamo-2");
const vanillish_1 = require("./vanillish");
const vanillite_1 = require("./vanillite");
const vanilluxe_1 = require("./vanilluxe");
const victini_1 = require("./victini");
const xtransceiver_1 = require("./xtransceiver");
const zweilous_1 = require("./zweilous");
const super_rod_1 = require("./super-rod");
exports.setNobleVictories = [
    new archen_1.Archen(),
    new archeops_1.Archeops(),
    new blitzle_1.Blitzle(),
    new chandelure_1.Chandelure(),
    new durant_1.Durant(),
    new eelektrik_1.Eelektrik(),
    new eviolite_1.Eviolite(),
    new kyurem_1.Kyurem(),
    new lampent_1.Lampent(),
    new palpitoad_1.Palpitoad(),
    new petilil_1.Petilil(),
    new plume_fossil_1.PlumeFossil(),
    new seismitoad_1.Seismitoad(),
    new super_rod_1.SuperRod(),
    new terrakion_1.Terrakion(),
    new trubbish_1.Trubbish(),
    new tympole_1.Tympole(),
    new tynamo_1.Tynamo(),
    new tynamo_2_1.Tynamo2(),
    new vanillish_1.Vanillish(),
    new vanillite_1.Vanillite(),
    new vanilluxe_1.Vanilluxe(),
    new victini_1.Victini(),
    new xtransceiver_1.Xtransceiver(),
    new zweilous_1.Zweilous(),
    new other_prints_1.CobalionNVI(),
    new other_prints_1.NNVI(),
    new other_prints_1.RockyHelmetNVI(),
    new other_prints_1.Terrakion2NVI(),
    new other_prints_1.Cobalion2NVI(),
    new other_prints_1.N2NVI(),
];
