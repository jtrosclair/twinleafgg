"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.setAquapolis = void 0;
const blissey_1 = require("./blissey");
const boost_energy_1 = require("./boost-energy");
const chansey_1 = require("./chansey");
const chinchou_1 = require("./chinchou");
const chinchou2_1 = require("./chinchou2");
const eevee_1 = require("./eevee");
const espeon_1 = require("./espeon");
const gloom_1 = require("./gloom");
const horsea_1 = require("./horsea");
const kingdra_1 = require("./kingdra");
const lugia_1 = require("./lugia");
const magnemite_1 = require("./magnemite");
const metal_energy_special_1 = require("./metal-energy-special");
const porygon_1 = require("./porygon");
const porygon2_1 = require("./porygon2");
const seadra_1 = require("./seadra");
const super_energy_removal_2_1 = require("./super-energy-removal-2");
const town_volunteers_1 = require("./town-volunteers");
const weakness_guard_1 = require("./weakness-guard");
const zapdos_1 = require("./zapdos");
// Other prints
const other_prints_1 = require("./other-prints");
const other_prints_2 = require("./other-prints");
const other_prints_3 = require("./other-prints");
exports.setAquapolis = [
    new blissey_1.Blissey(),
    new boost_energy_1.BoostEnergy(),
    new chansey_1.Chansey(),
    new chinchou_1.Chinchou(),
    new chinchou2_1.Chinchou2(),
    new eevee_1.Eevee(),
    new espeon_1.Espeon(),
    new gloom_1.Gloom(),
    new horsea_1.Horsea(),
    new kingdra_1.Kingdra(),
    new lugia_1.Lugia(),
    new magnemite_1.Magnemite(),
    new metal_energy_special_1.MetalEnergySpecial(),
    new porygon_1.Porygon(),
    new porygon2_1.Porygon2(),
    new seadra_1.Seadra(),
    new super_energy_removal_2_1.SuperEnergyRemoval2(),
    new town_volunteers_1.TownVolunteers(),
    new weakness_guard_1.WeaknessGuard(),
    new zapdos_1.Zapdos(),
    // Other prints
    new other_prints_1.PokemonFanClubAQ(),
    new other_prints_2.RainbowEnergyAQ(),
    new other_prints_3.WarpEnergyAQ(),
];
