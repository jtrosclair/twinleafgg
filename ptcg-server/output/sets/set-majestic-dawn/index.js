"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.setMajesticDawn = void 0;
const other_prints_1 = require("./other-prints");
const bronzong_1 = require("./bronzong");
const bronzor_1 = require("./bronzor");
const call_energy_1 = require("./call-energy");
const chatot_1 = require("./chatot");
const croagunk_1 = require("./croagunk");
const darkrai_1 = require("./darkrai");
const empoleon_1 = require("./empoleon");
const prinplup_1 = require("./prinplup");
const quick_ball_1 = require("./quick-ball");
const scizor_1 = require("./scizor");
const scyther_1 = require("./scyther");
const toxicroak_1 = require("./toxicroak");
const unown_p_1 = require("./unown-p");
const unown_q_1 = require("./unown-q");
// Other prints
const other_prints_2 = require("./other-prints");
exports.setMajesticDawn = [
    new bronzong_1.Bronzong(),
    new bronzor_1.Bronzor(),
    new call_energy_1.CallEnergy(),
    new chatot_1.Chatot(),
    new croagunk_1.Croagunk(),
    new darkrai_1.Darkrai(),
    new empoleon_1.Empoleon(),
    new prinplup_1.Prinplup(),
    new quick_ball_1.QuickBall(),
    new scizor_1.Scizor(),
    new scyther_1.Scyther(),
    new toxicroak_1.Toxicroak(),
    new unown_p_1.UnownP(),
    new unown_q_1.UnownQ(),
    // Other prints
    new other_prints_2.WarpPointMD(),
    new other_prints_1.DuskBallMD(),
    new other_prints_1.PokeBallMD(),
    new other_prints_1.SuperScoopUpMD(),
    new other_prints_1.EnergySearchMD(),
    new other_prints_1.DarknessEnergySpecialMD(),
    new other_prints_1.MetalEnergyN1MD(),
];
