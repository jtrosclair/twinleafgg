    ...
    reduceEffect(store, state, effect) {
        if (effect instanceof check_effects_1.CheckPokemonPowersEffect) {
            ...
            if (isGarbodorWithToolInPlay) {
                ...
                effect.powers = effect.powers.filter(power => power.powerType !== pokemon_types_1.PowerType.ABILITY || power.name === 'Garbotoxin');
            }
        }
        if (effect instanceof game_effects_1.PowerEffect
            ...
            if (cardList instanceof game_1.PokemonCardList) {
                const canApplyAbility = new game_effects_1.EffectOfAbilityEffect(isGarbodorWithToolInPlay ? player : opponent, this.powers[0], this, cardList);
