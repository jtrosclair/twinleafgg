...
class Windstorm extends trainer_card_1.TrainerCard {
    constructor() {
        super(...arguments);
        this.trainerType = card_types_1.TrainerType.ITEM;
        this.set = 'CG';
        this.name = 'Windstorm';
        this.fullName = 'Windstorm CG';
    ...
    processToolDiscards(store, state, player, targets, index, onComplete) {
        ...
        if (target.tools.length === 1) {
            ...
            const toolList = new game_1.CardList();
            toolList.cards = [...target.tools];
            store.prompt(state, new game_1.ChooseCardsPrompt(player, game_1.GameMessage.CHOOSE_CARD_TO_DISCARD, toolList, { trainerType: card_types_1.TrainerType.TOOL }, { min: 1, max: 1, allowCancel: false }), selectedTools => {
