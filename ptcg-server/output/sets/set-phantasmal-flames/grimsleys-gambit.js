...
class GrimsleysGambit extends game_1.TrainerCard {
    ...
    reduceEffect(store, state, effect) {
        if (effect instanceof play_card_effects_1.TrainerEffect && effect.trainerCard === this) {
            ...
            state = (0, prefabs_1.MOVE_CARDS)(store, state, player.deck, deckTop, { count: 7 });
            let cards = [];
            return store.prompt(state, new game_1.ChooseCardsPrompt(player, game_1.GameMessage.CHOOSE_CARD_TO_PUT_ONTO_BENCH, deckTop, { superType: game_1.SuperType.POKEMON, cardType: game_1.CardType.DARK }, { min: 0, max: 1, allowCancel: false }), selectedCards => {
                ...
                cards.forEach((card, index) => {
                    state = (0, prefabs_1.MOVE_CARDS)(store, state, deckTop, openSlots[index], { cards: [card] });
                    openSlots[index].pokemonPlayedTurn = state.turn;
                });
                state = store.prompt(state, new game_1.ShuffleDeckPrompt(player.id), order => {
                    deckTop.applyOrder(order);
                });
                state = (0, prefabs_1.MOVE_CARDS)(store, state, deckTop, player.deck, { toBottom: true });
                state = (0, prefabs_1.MOVE_CARDS)(store, state, player.supporter, player.discard, { cards: [effect.trainerCard] });
                return state;
            });
        }
        return state;
