...
At the end of this turn, if you have 3 or more cards in your hand, shuffle cards from your hand into your deck until you have 2 cards in your hand. `;
    ...
    reduceEffect(store, state, effect) {
        if (effect instanceof play_card_effects_1.TrainerEffect && effect.trainerCard === this) {
            ...
            prefabs_1.ADD_MARKER(this.LILLIES_FORCE_MARKER, effect.player, this);
        }
        if (effect instanceof game_phase_effects_1.EndTurnEffect && prefabs_1.HAS_MARKER(this.LILLIES_FORCE_MARKER, effect.player, this)) {
            if (effect.player.hand.cards.length >= 3) {
                ...
                return store.prompt(state, new game_1.ChooseCardsPrompt(effect.player, game_1.GameMessage.CHOOSE_CARD_TO_DISCARD, effect.player.hand, {}, { min: discardAmount, max: discardAmount, allowCancel: false }), selected => {
                    ...
                    prefabs_1.MOVE_CARDS(store, state, effect.player.hand, effect.player.deck, { cards, sourceCard: this });
                    prefabs_1.SHUFFLE_DECK(store, state, effect.player);
                });
            }
        }
        prefabs_1.REMOVE_MARKER_AT_END_OF_TURN(effect, this.LILLIES_FORCE_MARKER, this);
        return state;
    }
}
