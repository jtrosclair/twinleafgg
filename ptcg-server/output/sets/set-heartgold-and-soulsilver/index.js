"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.setHeartGoldAndSoulSilver = void 0;
const other_prints_1 = require("./other-prints");
const bayleef_1 = require("./bayleef");
const chikorita_1 = require("./chikorita");
const cleffa_1 = require("./cleffa");
const cyndaquil_1 = require("./cyndaquil");
const donphan_prime_1 = require("./donphan-prime");
const drowzee_1 = require("./drowzee");
const grass_energy_1 = require("./grass-energy");
const hoppip_1 = require("./hoppip");
const jumpluff_1 = require("./jumpluff");
const fire_energy_1 = require("./fire-energy");
const water_energy_1 = require("./water-energy");
const lightning_energy_1 = require("./lightning-energy");
const phanpy_1 = require("./phanpy");
const psychic_energy_1 = require("./psychic-energy");
const fighting_energy_1 = require("./fighting-energy");
const darkness_energy_1 = require("./darkness-energy");
const meganium_prime_1 = require("./meganium-prime");
const metal_energy_1 = require("./metal-energy");
const ninetales_1 = require("./ninetales");
const hypno_1 = require("./hypno");
const pichu_1 = require("./pichu");
const pokemon_collector_1 = require("./pokemon-collector");
const professor_oaks_new_theory_1 = require("./professor-oaks-new-theory");
const quilava_1 = require("./quilava");
const typhlosion_1 = require("./typhlosion");
const tyrogue_1 = require("./tyrogue");
const unown_1 = require("./unown");
const vulpix_1 = require("./vulpix");
// Other prints
const other_prints_2 = require("./other-prints");
exports.setHeartGoldAndSoulSilver = [
    new bayleef_1.Bayleef(),
    new chikorita_1.Chikorita(),
    new cleffa_1.Cleffa(),
    new cyndaquil_1.Cyndaquil(),
    new darkness_energy_1.DarknessEnergy(),
    new donphan_prime_1.Donphan(),
    new drowzee_1.Drowzee(),
    new fighting_energy_1.FightingEnergy(),
    new fire_energy_1.FireEnergy(),
    new grass_energy_1.GrassEnergy(),
    new hoppip_1.Hoppip(),
    new hypno_1.Hypno(),
    new jumpluff_1.Jumpluff(),
    new lightning_energy_1.LightningEnergy(),
    new meganium_prime_1.Meganium(),
    new metal_energy_1.MetalEnergy(),
    new ninetales_1.Ninetales(),
    new phanpy_1.Phanpy(),
    new pichu_1.Pichu(),
    new pokemon_collector_1.PokemonCollector(),
    new professor_oaks_new_theory_1.ProfessorOaksNewTheory(),
    new psychic_energy_1.PsychicEnergy(),
    new quilava_1.Quilava(),
    new typhlosion_1.Typhlosion(),
    new tyrogue_1.Tyrogue(),
    new unown_1.Unown(),
    new vulpix_1.Vulpix(),
    new water_energy_1.WaterEnergy(),
    // Other prints
    new other_prints_2.DoubleColorlessEnergyHS(),
    new other_prints_2.FishermanHS(),
    new other_prints_2.Pokegear30HS(),
    new other_prints_2.PokemonCommunicationHS(),
    new other_prints_2.PokemonReversalHS(),
    new other_prints_2.RainbowEnergyHS(),
    new other_prints_2.SwitchHS(),
    new other_prints_1.BillHS(),
    new other_prints_1.CopycatHS(),
    new other_prints_1.EnergySwitchPKHS(),
    new other_prints_1.FullHealHS(),
    new other_prints_1.PokeBallHS(),
    new other_prints_1.ProfessorElmsTrainingMethodHS(),
];
