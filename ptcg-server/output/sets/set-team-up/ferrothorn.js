...
class Ferrothorn extends pokemon_card_1.PokemonCard {
    constructor() {
        ...
        this.fullName = 'Ferrothorn TEU';
        this.legacyFullName = 'Ferrothorn UNB';
    ...
    reduceEffect(store, state, effect) {
        ...
        if (effect instanceof game_effects_1.AttackEffect && effect.attack === this.attacks[1]) {
            ...
            checkProvidedEnergy.energyMap.forEach(energy => {
                if (energy.provides.includes(card_types_1.CardType.METAL) || energy.provides.includes(card_types_1.CardType.ANY)) {
                    if (snipeDamage < 100) {
                        snipeDamage += 20;
                    }
                }
            ...
            state = store.prompt(state, new game_1.ChoosePokemonPrompt(player.id, game_1.GameMessage.CHOOSE_POKEMON_TO_DAMAGE, game_1.PlayerType.TOP_PLAYER, [game_1.SlotType.BENCH], { allowCancel: false }), targets => {
                ...
                const damageEffect = new attack_effects_1.PutDamageEffect(effect, snipeDamage);
