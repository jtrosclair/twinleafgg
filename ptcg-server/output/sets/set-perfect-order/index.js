"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.setPerfectOrder = void 0;
const barbaracle_1 = require("./barbaracle");
const binacle_1 = require("./binacle");
const core_memory_1 = require("./core-memory");
const lumioise_city_1 = require("./lumioise-city");
const mega_skarmory_ex_1 = require("./mega-skarmory-ex");
const mega_zygarde_ex_1 = require("./mega-zygarde-ex");
const meowth_ex_1 = require("./meowth-ex");
const naveen_1 = require("./naveen");
const rock_fighting_energy_1 = require("./rock-fighting-energy");
const tarragon_1 = require("./tarragon");
const scatterbug_1 = require("./scatterbug");
const spewpa_1 = require("./spewpa");
const vivillon_1 = require("./vivillon");
const rowlet_1 = require("./rowlet");
const dartrix_1 = require("./dartrix");
const decidueye_ex_1 = require("./decidueye-ex");
const seel_1 = require("./seel");
const dewgong_1 = require("./dewgong");
const staryu_1 = require("./staryu");
const mega_starmie_ex_1 = require("./mega-starmie-ex");
const grow_grass_energy_1 = require("./grow-grass-energy");
const antique_jaw_fossil_1 = require("./antique-jaw-fossil");
const antique_sail_fossil_1 = require("./antique-sail-fossil");
const tyrunt_1 = require("./tyrunt");
const tyrantrum_1 = require("./tyrantrum");
const amaura_1 = require("./amaura");
const aurorus_1 = require("./aurorus");
const spinarak_1 = require("./spinarak");
const ariados_1 = require("./ariados");
const snivy_1 = require("./snivy");
const servine_1 = require("./servine");
const serperior_1 = require("./serperior");
const fletchinder_1 = require("./fletchinder");
const talonflame_1 = require("./talonflame");
const salandit_1 = require("./salandit");
const salazzle_ex_1 = require("./salazzle-ex");
const turtonator_1 = require("./turtonator");
const shinx_1 = require("./shinx");
const luxio_1 = require("./luxio");
const luxray_1 = require("./luxray");
const dedenne_1 = require("./dedenne");
const honedge_1 = require("./honedge");
const doublade_1 = require("./doublade");
const aegislash_1 = require("./aegislash");
const energy_swatter_1 = require("./energy-swatter");
const lumiose_galette_1 = require("./lumiose-galette");
const rosas_encouragement_1 = require("./rosas-encouragement");
const jacinthe_1 = require("./jacinthe");
const telepath_psychic_energy_1 = require("./telepath-psychic-energy");
const hawlucha_1 = require("./hawlucha");
const yveltal_ex_1 = require("./yveltal-ex");
const clefairy_1 = require("./clefairy");
const mega_clefable_ex_1 = require("./mega-clefable-ex");
const spritzee_1 = require("./spritzee");
const aromatisse_1 = require("./aromatisse");
const klefki_1 = require("./klefki");
const rattata_1 = require("./rattata");
const raticate_1 = require("./raticate");
const snorlax_1 = require("./snorlax");
const bunnelby_1 = require("./bunnelby");
const diggersby_1 = require("./diggersby");
const fletchling_1 = require("./fletchling");
const furfrou_1 = require("./furfrou");
const shaymin_1 = require("./shaymin");
const volcanion_1 = require("./volcanion");
const mawile_1 = require("./mawile");
const espurr_1 = require("./espurr");
const meowstic_1 = require("./meowstic");
const nosepass_1 = require("./nosepass");
const probopass_1 = require("./probopass");
const hippopotas_1 = require("./hippopotas");
const hippowdon_1 = require("./hippowdon");
const landorus_1 = require("./landorus");
const gastly_1 = require("./gastly");
const haunter_1 = require("./haunter");
const gengar_1 = require("./gengar");
const skorupi_1 = require("./skorupi");
const drapion_1 = require("./drapion");
const chien_pao_1 = require("./chien-pao");
const lapras_ex_1 = require("./lapras-ex");
const hole_digging_shovel_1 = require("./hole-digging-shovel");
const other_prints_1 = require("./other-prints");
exports.setPerfectOrder = [
    new barbaracle_1.Barbaracle(),
    new binacle_1.Binacle,
    new core_memory_1.CoreMemory(),
    new lumioise_city_1.LumioiseCity(),
    new mega_skarmory_ex_1.MegaSkarmoryex(),
    new mega_zygarde_ex_1.MegaZygardeex(),
    new meowth_ex_1.Meowthex(),
    new naveen_1.Naveen(),
    new rock_fighting_energy_1.RockFightingEnergy(),
    new tarragon_1.Tarragon(),
    new scatterbug_1.Scatterbug(),
    new spewpa_1.Spewpa(),
    new vivillon_1.Vivillon(),
    new rowlet_1.Rowlet(),
    new dartrix_1.Dartrix(),
    new decidueye_ex_1.Decidueyeex(),
    new seel_1.Seel(),
    new dewgong_1.Dewgong(),
    new staryu_1.Staryu(),
    new mega_starmie_ex_1.MegaStarmieex(),
    new grow_grass_energy_1.GrowGrassEnergy(),
    new antique_jaw_fossil_1.AntiqueJawFossil(),
    new antique_sail_fossil_1.AntiqueSailFossil(),
    new tyrunt_1.Tyrunt(),
    new tyrantrum_1.Tyrantrum(),
    new amaura_1.Amaura(),
    new aurorus_1.Aurorus(),
    new spinarak_1.Spinarak(),
    new ariados_1.Ariados(),
    new snivy_1.Snivy(),
    new servine_1.Servine(),
    new serperior_1.Serperior(),
    new fletchinder_1.Fletchinder(),
    new talonflame_1.Talonflame(),
    new salandit_1.Salandit(),
    new salazzle_ex_1.Salazzleex(),
    new turtonator_1.Turtonator(),
    new shinx_1.Shinx(),
    new luxio_1.Luxio(),
    new luxray_1.Luxray(),
    new dedenne_1.Dedenne(),
    new honedge_1.Honedge(),
    new doublade_1.Doublade(),
    new aegislash_1.Aegislash(),
    new energy_swatter_1.EnergySwatter(),
    new lumiose_galette_1.LumioseGalette(),
    new rosas_encouragement_1.RosasEncouragement(),
    new jacinthe_1.Jacinthe(),
    new telepath_psychic_energy_1.TelepathPsychicEnergy(),
    new hawlucha_1.Hawlucha(),
    new yveltal_ex_1.Yveltalex(),
    new clefairy_1.Clefairy(),
    new mega_clefable_ex_1.MegaClefableex(),
    new spritzee_1.Spritzee(),
    new aromatisse_1.Aromatisse(),
    new klefki_1.Klefki(),
    new rattata_1.Rattata(),
    new raticate_1.Raticate(),
    new snorlax_1.Snorlax(),
    new bunnelby_1.Bunnelby(),
    new diggersby_1.Diggersby(),
    new fletchling_1.Fletchling(),
    new furfrou_1.Furfrou(),
    new shaymin_1.Shaymin(),
    new volcanion_1.Volcanion(),
    new mawile_1.Mawile(),
    new espurr_1.Espurr(),
    new meowstic_1.Meowstic(),
    new nosepass_1.Nosepass(),
    new probopass_1.Probopass(),
    new hippopotas_1.Hippopotas(),
    new hippowdon_1.Hippowdon(),
    new landorus_1.Landorus(),
    new gastly_1.Gastly(),
    new haunter_1.Haunter(),
    new gengar_1.Gengar(),
    new skorupi_1.Skorupi(),
    new drapion_1.Drapion(),
    new chien_pao_1.ChienPao(),
    new lapras_ex_1.Laprasex(),
    new hole_digging_shovel_1.HoleDiggingShovel(),
    // IRs, FAs, SIRs, HR
    new other_prints_1.SpewpaIR(),
    new other_prints_1.RowletIR(),
    new other_prints_1.TalonflameIR(),
    new other_prints_1.AurorusIR(),
    new other_prints_1.DedenneIR(),
    new other_prints_1.ClefairyIR(),
    new other_prints_1.EspurrIR(),
    new other_prints_1.ProbopassIR(),
    new other_prints_1.DrapionIR(),
    new other_prints_1.DoubladeIR(),
    new other_prints_1.RaticateIR(),
    new other_prints_1.DecidueyeexFA(),
    new other_prints_1.SalazzleexFA(),
    new other_prints_1.MegaStarmieexFA(),
    new other_prints_1.MegaClefableexFA(),
    new other_prints_1.MegaZygardeexFA(),
    new other_prints_1.YveltalexFA(),
    new other_prints_1.MegaSkarmoryexFA(),
    new other_prints_1.MeowthexFA(),
    new other_prints_1.EnergyRecyclerFA(),
    new other_prints_1.SacredAshFA(),
    new other_prints_1.PokePadNormal(),
    new other_prints_1.PokePadFA(),
    new other_prints_1.WondrousPatchFA(),
    new other_prints_1.TarragonFA(),
    new other_prints_1.NaveenFA(),
    new other_prints_1.RosasEncouragementFA(),
    new other_prints_1.JacintheFA(),
    new other_prints_1.ForestOfVitalityFA(),
    new other_prints_1.LumioiseCityFA(),
    new other_prints_1.MegaStarmieexSIR(),
    new other_prints_1.MegaClefableexSIR(),
    new other_prints_1.MegaZygardeexSIR(),
    new other_prints_1.MeowthexSIR(),
    new other_prints_1.RosasEncouragementSIR(),
    new other_prints_1.JacintheSIR(),
    new other_prints_1.MegaZygardeexHR(),
    new other_prints_1.JudgePOR()
];
