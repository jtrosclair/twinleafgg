...
function* useLineUp(next, store, state, effect) {
    ...
    let cards = [];
    yield store.prompt(state, new game_1.ChooseCardsPrompt(player, game_1.GameMessage.CHOOSE_CARD_TO_PUT_ONTO_BENCH, player.deck, { superType: card_types_1.SuperType.POKEMON, name: 'Charjabug' }, { min: 0, max, allowCancel: false }), selected => {
        cards = selected || [];
        next();
    });
