...
class Gholdengoex extends pokemon_card_1.PokemonCard {
    ...
    reduceEffect(store, state, effect) {
        ...
        if (effect instanceof game_effects_1.AttackEffect && effect.attack === this.attacks[0]) {
            ...
            return store.prompt(state, new game_1.ChooseCardsPrompt(player, game_message_1.GameMessage.CHOOSE_CARD_TO_DISCARD, player.hand, { superType: card_types_1.SuperType.ENERGY }, { allowCancel: false, min: 0, max: energiesInHand.length }), cards => {
                cards = cards || [];
                if (cards.length === 0) {
                    return;
                }
