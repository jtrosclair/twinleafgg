...
function formatPowers(powers) {
    ...
    return '[\n' + powers.map(p => {
        var _a, _b;
        return `    {
      name: '${((_a = p.name) === null || _a === void 0 ? void 0 : _a.en) || p.name || ''}',
      powerType: '${p.type || ''}',
      text: '${typeof p.effect === 'string' ? p.effect.replace(/'/g, '\\\'').replace(/\n/g, '\\n') : (((_b = p.effect) === null || _b === void 0 ? void 0 : _b.en) ? p.effect.en.replace(/'/g, '\\\'').replace(/\n/g, '\\n') : '')}'
    }`;
    }).join(',\n') + '\n  ]';
}
...
function formatAttacksRaw(attacks) {
    ...
    return attacks.map(a => {
        ...
        let text = '';
        if (a.effect) {
            text = typeof a.effect === 'string' ? a.effect : (((_b = a.effect) === null || _b === void 0 ? void 0 : _b.en) || '');
        }
...
function getTags(card) {
    const tags = [];
    if (card.stage && card.stage.toLowerCase() !== 'basic')
        tags.push('CardTag.POKEMON_ex'); // Example, adjust as needed
...
async function main() {
    ...
    const className = toPascalCase(((_a = card.name) === null || _a === void 0 ? void 0 : _a.en) || card.name);
    const fileName = toKebabCase(((_b = card.name) === null || _b === void 0 ? void 0 : _b.en) || card.name) + '.ts';
    const { baseClass, importPath } = getBaseClassAndImport(card.category || ((_c = card.set) === null || _c === void 0 ? void 0 : _c.category));
    const setAbbr = ((_e = (_d = card.set) === null || _d === void 0 ? void 0 : _d.abbreviations) === null || _e === void 0 ? void 0 : _e.official) || ((_f = card.set) === null || _f === void 0 ? void 0 : _f.tcgOnline) || (typeof card.set === 'string' ? card.set : undefined);
    ...
    if (card.evolveFrom || card.evolvesFrom) {
        props += `  public evolvesFrom = '${escapeSingleQuotes(((_g = card.evolveFrom) === null || _g === void 0 ? void 0 : _g.en) || card.evolveFrom || card.evolvesFrom)}';\n`;
    }
    props += `  public cardType = ${mapCardTypeSymbol((card.cardType || ((_h = card.types) === null || _h === void 0 ? void 0 : _h[0]) || (card.type && card.type[0])))};\n`;
    ...
    props += `  public name: string = '${escapeSingleQuotes(((_j = card.name) === null || _j === void 0 ? void 0 : _j.en) || card.name)}';\n`;
    props += `  public fullName: string = '${escapeSingleQuotes(((_k = card.name) === null || _k === void 0 ? void 0 : _k.en) || card.name)} ${(setAbbr || '').toUpperCase()}';\n`;
