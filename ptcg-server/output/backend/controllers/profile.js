...
class Profile extends controller_1.Controller {
    ...
    async onGetCardImagesUrl(req, res) {
    ...
    async onSetCardImagesUrl(req, res) {
        const userId = req.body.userId;
        const body = req.body;
        const user = await storage_1.User.findOne(userId);
        if (user === undefined) {
    ...
    async onUpdateRole(req, res) {
        const adminId = req.body.userId;
        const admin = await storage_1.User.findOne(adminId);
        if (!admin || admin.roleId !== 4) {
            res.status(403);
            res.send({ error: errors_1.ApiErrorEnum.AUTH_INVALID_PERMISSIONS });
            return;
        }
        const body = req.body;
        const targetUser = await storage_1.User.findOne(body.targetUserId);
        if (!targetUser) {
            res.status(400);
            res.send({ error: errors_1.ApiErrorEnum.PROFILE_INVALID });
            return;
...
__decorate([
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], Profile.prototype, "onUpdateRole", null);
