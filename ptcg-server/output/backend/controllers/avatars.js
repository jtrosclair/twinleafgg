...
class Avatars extends controller_1.Controller {
    async onGetAvailable(req, res) {
        ...
        const unlockedItems = await storage_1.UserUnlockedItem.find({ where: { userId, itemType: 'avatar' } });
        const unlockedAvatars = unlockedItems.map(item => {
