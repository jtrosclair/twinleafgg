...
exports.ReconnectionCleanupService = void 0;
const disconnected_session_1 = require("../../storage/model/disconnected-session");
const logger_1 = require("../../utils/logger");
const scheduler_1 = require("../../utils/scheduler");
class ReconnectionCleanupService {
    constructor(gameStatePreserver, configManager, maintenanceConfig) {
        this.cleanupInterval = null;
        this.maintenanceInterval = null;
        this.isShuttingDown = false;
        this.activeCleanupPromises = [];
        this.metrics = {
        ...
        if (maintenanceConfig) {
            this.config = Object.assign(Object.assign({}, this.config), maintenanceConfig);
        }
    ...
    async performScheduledCleanup() {
        ...
        try {
            ...
            logger_1.logger.logCleanupOperation('scheduled-cleanup', expiredCount + orphanedCount, duration);
            logger_1.logger.logStructured({
                level: logger_1.LogLevel.INFO,
                category: 'cleanup',
    ...
    async cleanupExpiredSessions() {
        ...
        try {
            ...
            const duration = Date.now() - startTime;
            logger_1.logger.logCleanupOperation('expired-sessions', cleanedCount, duration);
            return cleanedCount;
        }
        catch (error) {
            const duration = Date.now() - startTime;
     ...
     */
    async cleanupOrphanedGameStates() {
        const startTime = Date.now();
        try {
            ...
            const duration = Date.now() - startTime;
            logger_1.logger.logCleanupOperation('orphaned-game-states', cleanedCount, duration);
            return cleanedCount;
        }
        catch (error) {
            const duration = Date.now() - startTime;
    ...
    async cleanupOldSessions() {
        ...
        try {
            ...
            const oldSessions = await disconnected_session_1.DisconnectedSession.createQueryBuilder('session')
            ...
            const duration = Date.now() - startTime;
            logger_1.logger.logCleanupOperation('old-sessions', cleanedCount, duration);
            return cleanedCount;
        }
        catch (error) {
            const duration = Date.now() - startTime;
        ...
        try {
            for (const tableName of tablesToOptimize) {
                ...
                try {
                    ...
                    await disconnected_session_1.DisconnectedSession.query(`OPTIMIZE TABLE ${tableName}`);
    ...
    async forceCleanupUserSessions(userId) {
        const startTime = Date.now();
        try {
            const userSessions = await disconnected_session_1.DisconnectedSession.find({
                where: { userId }
            });
            let cleanedCount = 0;
            ...
            const duration = Date.now() - startTime;
            logger_1.logger.logCleanupOperation(`force-cleanup-user-${userId}`, cleanedCount, duration);
            return cleanedCount;
        }
        catch (error) {
            const duration = Date.now() - startTime;
...
exports.ReconnectionCleanupService = ReconnectionCleanupService;
