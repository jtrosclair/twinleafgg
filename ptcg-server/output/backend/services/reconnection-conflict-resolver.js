...
exports.ReconnectionConflictResolver = void 0;
const state_1 = require("../../game/store/state/state");
const card_list_1 = require("../../game/store/state/card-list");
const logger_1 = require("../../utils/logger");
class ReconnectionConflictResolver {
    /**
     * Detect conflicts between server state and preserved state
     */
    detectConflicts(serverState, preservedState, gameId, userId) {
     ...
     */
    resolveConflicts(serverState, preservedState, conflicts, gameId, userId) {
        const sessionId = `conflict-resolution-${userId}-${gameId}-${Date.now()}`;
        let resolvedState = Object.assign({}, serverState); // Start with server state as base
     ...
     */
    validateState(state, gameId, userId) {
     ...
     */
    recoverCorruptedState(corruptedState, fallbackState, gameId, userId) {
        var _a;
        ...
        try {
            logger_1.logger.logStructured({
                ...
                data: {
                    ...
                    corruptedPhase: ((_a = corruptedState === null || corruptedState === void 0 ? void 0 : corruptedState.phase) === null || _a === void 0 ? void 0 : _a.toString()) || 'unknown'
                }
            });
     ...
     */
    applyFallbackStrategy(serverState, preservedState, conflicts, gameId, userId) {
     ...
     */
    resolvePhaseConflict(serverState, preservedState, conflict) {
        ...
        const resolvedState = Object.assign({}, serverState);
        // If the preserved state shows the player was in their turn,
        // we might need to restore some turn-specific information
        if (conflict.preservedState.phase === state_1.GamePhase.PLAYER_TURN &&
            conflict.serverState.phase === state_1.GamePhase.PLAYER_TURN) {
     ...
     */
    resolveTurnConflict(serverState, preservedState, conflict) {
     ...
     */
    resolveVersionConflict(serverState, preservedState, conflict) {
        // For version conflicts, always use the server state
        // Version mismatches typically indicate the preserved state is outdated
        return Object.assign({}, serverState);
    }
    /**
     * Check for data corruption indicators
     */
    checkForDataCorruption(serverState, preservedState) {
        try {
     ...
     */
    getPhaseConflictSeverity(serverPhase, preservedPhase) {
        // If game is finished on server but not in preserved state, it's high severity
        if (serverPhase === state_1.GamePhase.FINISHED && preservedPhase !== state_1.GamePhase.FINISHED) {
            return 'high';
        }
        // If preserved state shows finished but server doesn't, it's critical
        if (preservedPhase === state_1.GamePhase.FINISHED && serverPhase !== state_1.GamePhase.FINISHED) {
            return 'critical';
        }
     ...
     */
    isPhaseConflictResolvable(serverPhase, preservedPhase) {
     ...
     */
    isValidGamePhase(phase) {
        return Object.values(state_1.GamePhase).includes(phase);
    }
    /**
    ...
    canRecoverFromErrors(errors) {
        ...
        const hasUnrecoverableErrors = errors.some(error => unrecoverableErrorPatterns.some(pattern => pattern.test(error)));
        if (hasUnrecoverableErrors) {
            return false;
        }
        // If all errors match recoverable patterns, return true
        return errors.every(error => recoverableErrorPatterns.some(pattern => pattern.test(error)));
    }
    /**
     * Attempt to repair corrupted state
     */
    attemptStateRepair(corruptedState, gameId, userId) {
        try {
            const repairedState = Object.assign({}, corruptedState);
            ...
            for (let i = 0; i < repairedState.players.length; i++) {
                ...
                if (player) {
                    if (!player.deck || !(player.deck instanceof card_list_1.CardList)) {
                        player.deck = new card_list_1.CardList();
                    }
                    if (!player.hand || !(player.hand instanceof card_list_1.CardList)) {
                        player.hand = new card_list_1.CardList();
                    }
                    if (!player.discard || !(player.discard instanceof card_list_1.CardList)) {
                        player.discard = new card_list_1.CardList();
                    }
                    if (!Array.isArray(player.prizes)) {
                        player.prizes = [];
                    }
...
exports.ReconnectionConflictResolver = ReconnectionConflictResolver;
