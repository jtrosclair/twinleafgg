...
import { AttackEffect, EvolveEffect, HealEffect, KnockOutEffect, PowerEffect, PutDamageCountersEffect, TrainerPowerEffect, UseAttackEffect, UsePowerEffect, UseStadiumEffect, UseTrainerPowerEffect } from '../effects/game-effects';
    ...
    if ((attack.barrage || hasBarragePower) && !effect._barrageUsed) {
        ...
        if (wantToUse) {
            ...
            if (!attack.barrage && hasBarragePower) {
                ...
                yield store.prompt(state, new ChooseAttackPrompt(player.id, GameMessage.CHOOSE_ATTACK_TO_COPY, attackableCards, { allowCancel: false }), (selectedAttack) => {
                    if (selectedAttack) {
...
export function gameReducer(store, state, effect) {
    ...
    if (effect instanceof EvolveEffect) {
        ...
        GameStatsTracker.handlePokemonEvolution(effect.player, effect.target, pokemonCard, effect.pokemonCard);
