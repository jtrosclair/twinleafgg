...
const clear_effect_markers_1 = require("../prefabs/clear-effect-markers");
class Marker {
    constructor() {
        this.markers = [];
    }
    ...
    removeAttackEffects() {
        this.markers = this.markers.filter(m => {
            if (m.sourceType === 'attack')
                return false;
            if (clear_effect_markers_1.KNOWN_ATTACK_EFFECT_MARKERS.has(m.name))
                return false;
            return true;
        });
    }
    ...
    removePokemonScopedMarkers() {
        this.markers = this.markers.filter(m => {
            if (m.targetScope === 'pokemon')
                return false;
            if (clear_effect_markers_1.KNOWN_POKEMON_SCOPED_PLAYER_MARKERS.has(m.name))
                return false;
            return true;
        });
    }
