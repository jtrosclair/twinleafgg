...
export const AFTER_ATTACK = (effect, index, user) => {
...
export function DEVOLVE_POKEMON(store, state, target, destination) {
    ...
    if (pokemonCard === null || pokemonCard === void 0 ? void 0 : pokemonCard.tags.includes(CardTag.POKEMON_LV_X)) {
    ...
    if (pokemons.length > 1 && !(pokemonCard === null || pokemonCard === void 0 ? void 0 : pokemonCard.tags.includes(CardTag.POKEMON_VUNION)) && !(pokemonCard === null || pokemonCard === void 0 ? void 0 : pokemonCard.tags.includes(CardTag.LEGEND))) {
        MOVE_CARD_TO(state, pokemonCard, destination);
        target.clearEffects();
        target.pokemonPlayedTurn = state.turn;
    }
}
export function THIS_ATTACK_DOES_X_DAMAGE_TO_X_OF_YOUR_OPPONENTS_POKEMON(damage, effect, store, state, min, max, applyWeaknessAndResistance = false, slots) {
    const player = effect.player;
    const opponent = StateUtils.getOpponent(state, player);
    ...
    return store.prompt(state, new ChoosePokemonPrompt(player.id, GameMessage.CHOOSE_POKEMON_TO_DAMAGE, PlayerType.TOP_PLAYER, slots !== null && slots !== void 0 ? slots : [SlotType.BENCH], { min: min, max: max, allowCancel: false }), selected => {
...
export function DAMAGE_OPPONENT_POKEMON(store, state, effect, damage, targets) {
    const player = effect.player;
    const opponent = StateUtils.getOpponent(state, player);
 ...
 */
export function DRAW_UP_TO_X_CARDS(store, state, player, count) {
    if (count > 0) {
        const options = [];
        for (let i = count; i >= 0; i--) {
            options.push({ message: `Draw ${i} card(s)`, value: i });
        }
        store.prompt(state, new SelectPrompt(player.id, GameMessage.WANT_TO_DRAW_CARDS, options.map(c => c.message), { allowCancel: false }), choice => {
...
export function SEARCH_DECK_FOR_CARDS_TO_HAND(store, state, player, sourceCard, filter = {}, options = {}, sourceEffect) {
    if (player.deck.cards.length === 0)
        return;
    const opponent = StateUtils.getOpponent(state, player);
    store.prompt(state, new ChooseCardsPrompt(player, GameMessage.CHOOSE_CARD_TO_HAND, player.deck, filter, options), selected => {
        const cards = selected || [];
...
export function CLEAN_UP_SUPPORTER(effect, player) {
 ...
 * A `filter` can be provided for the prompt as well.
 */
export function SEARCH_DISCARD_PILE_FOR_CARDS_TO_HAND(store, state, player, sourceCard, filter = {}, options = {}, sourceEffect) {
    if (player.discard.cards.length === 0)
        return;
    const opponent = StateUtils.getOpponent(state, player);
    store.prompt(state, new ChooseCardsPrompt(player, GameMessage.CHOOSE_CARD_TO_HAND, player.discard, filter, options), selected => {
        const cards = selected || [];
        cards.forEach(card => {
            store.log(state, GameLog.LOG_PLAYER_PUTS_CARD_IN_HAND, { name: player.name, card: card.name });
        });
 ...
 */
export function IS_POKEBODY_BLOCKED(store, state, player, card) {
 ...
 */
export function IS_POKEPOWER_BLOCKED(store, state, player, card) {
 ...
 */
export function IS_POKEMON_POWER_BLOCKED(store, state, player, card) {
 ...
 */
export function IS_SPECIAL_ENERGY_BLOCKED(store, state, player, card, attachedTo, exemptFromOpponentsSpecialEnergyBlockingAbility = false) {
...
export function SWITCH_ACTIVE_WITH_BENCHED(store, state, player) {
    ...
    if (!hasBenched)
        return state;
    store.prompt(state, new ChoosePokemonPrompt(player.id, GameMessage.CHOOSE_NEW_ACTIVE_POKEMON, PlayerType.BOTTOM_PLAYER, [SlotType.BENCH], { allowCancel: false }), selected => {
...
export function BLOCK_IF_HAS_SPECIAL_CONDITION(player, source) {
    if (player.active.getPokemonCard() === source && player.active.specialConditions.length > 0)
        throw new GameError(GameMessage.CANNOT_USE_POWER);
}
export function BLOCK_IF_ASLEEP_CONFUSED_PARALYZED(player, source) {
 ...
 */
export function CAN_PLAY_SUPPORTER_CARD(store, state, player, trainerCard) {
    try {
 ...
 */
export function BLOCK_RETREAT(store, state, effect, source) {
 ...
 * @returns The updated game state
 */
export function PREVENT_DAMAGE(store, state, effect, source) {
 ...
 */
export function DAMAGED_FROM_FULL_HP(store, state, effect, player, target) {
 ...
 * @param source The source card that created this effect
 */
export function TERA_RULE(effect, state, source) {
