import { BudewASC, TeamRocketsTarountulaASC, TeamRocketsSpidopsASC, EthansSlugmaASC, EthansMagcargoASC, EthansHoOhexASC, NsDarumakaASC, NsDarmanitanASC, SalanditASC, SalazzleASC, ScorbunnyASC, RabootASC, CinderaceexASC, PsyduckASC, GolduckASC, SneaselASC, WeavileASC, PikachuexASC, TynamoASC, EelektrikASC, CharjabugASC, IonosTadbulbASC, IonosBelliboltexASC, IonosWattrelASC, IonosKilowattrelASC, ClefairyASC, ClefableASC, LilliesClefairyexASC, TeamRocketsMewtwoexASC, TogepiASC, TogeticASC, TogekissASC, RaltsASC, KirliaASC, MegaGardevoirexASC, ShuppetASC, BanetteASC, TeamRocketsMimikyuASC, MunkidoriASC, LunatoneASC, SolrockASC, RegirockexASC, CynthiasGibleASC, CynthiasGabiteASC, CynthiasGarchompexASC, RioluASC, MegaLucarioexASC, GastlyASC, HaunterASC, MegaGengarexASC, TeamRocketsMurkrowASC, CynthiasSpiritombASC, NsZoruaASC, NsZoroarkexASC, FezandipitiexASC, NsReshiramASC, DreepyASC, DrakloakASC, DragapultexASC, TeamRocketsMeowthASC, FanRotomASC, HopsCramorantASC, TerapagosASC, TerapagosexASC, AirBalloonASC, BuddyBuddyPoffinASC, CounterGainASC, LushForestASC, GlassTrumpetASC, IrisFightingSpiritASC, LilliesDeterminationASC, MegaSignalASC, MysteryGardenASC, NsPPUpASC, NightlyStretcherASC, PowerProteinASC, SurferASC, TeamRocketsArcherASC, TeamRocketsArianaASC, TeamRocketsFactoryASC, TeamRocketsGiovanniASC, TeamRocketsGreatBallASC, TeamRocketsPetrelASC, TeamRocketsProtonASC, TeamRocketsTransceiverASC, TeamRocketsWatchtowerASC, ToolScrapperASC, UltraBallASC, PrismEnergyASC, TeamRocketsEnergyASC, ErikasTangela2ASC, Beautifly2ASC, Dustox2ASC, Budew2ASC, EthansMagcargo2ASC, Numel2ASC, Salazzle2ASC, Scorbunny2ASC, Psyduck2ASC, Snorunt2ASC, Weavile2ASC, IonosWattrel2ASC, Marill2ASC, Misdreavus2ASC, Banette2ASC, Togekiss2ASC, TeamRocketsMimikyu2ASC, CynthiasSpiritomb2ASC, Dreepy2ASC, Drakloak2ASC, LarrysStaraptor2ASC, FanRotom2ASC, MegaAudinoex2ASC, BlackBeltsTrainingASC, BossOrders2ASC, Canari2ASC, CherenASC, CounterGain2ASC, GlassTrumpet2ASC, JammingTowerASC, NsPPUp2ASC, TeamRocketsTransceiver2ASC, UltraBall2ASC, MegaFroslassex2ASC, MegaEelektrossex2ASC, MegaDiancieexASC, MegaHawluchaex2ASC, MegaGengarex2ASC, MegaScraftyex2ASC, MegaDragoniteex2ASC, MegaMeganiumex2ASC, MegaEmboarex2ASC, MegaFeraligatrex2ASC, MegaFroslassex3ASC, Pikachuex3ASC, MegaEelektrossex3ASC, IonosBelliboltex2ASC, LilliesClefairyex2ASC, TeamRocketsMewtwoex2ASC, MegaDiancieex2ASC, MegaHawluchaex3ASC, MegaGengarex3ASC, MegaScraftyex3ASC, NsZoroarkex2ASC, MarniesGrimmsnarlexASC, Fezandipitiex2ASC, StevensMetagrossexASC, MegaDragoniteex3ASC, Canari3ASC, IrisFightingSpirit2ASC, Surfer2ASC, MegaCharizardYex2ASC, MegaDragoniteex4ASC } from './other-prints';
import { Card } from '../../game/store/card/card';
import { AntheaAndConcordia } from './anthea-and-concordia';
import { Beautifly } from './beautifly';
import { Cascoon } from './cascoon';
import { Camerupt } from './camerupt';
import { Canari } from './canari';
import { Dragonair } from './dragonair';
import { Dratini } from './dratini';
import { Dustox } from './dustox';
import { Frosmoth } from './frosmoth';
import { MegaCharizardYex } from './mega-charizard-y-ex';
import { MegaDragoniteex } from './mega-dragonite-ex';
import { MegaEelektrossex } from './mega-eelektross-ex';
import { MegaFroslassex } from './mega-froslass-ex';
import { MegaHawluchaex } from './mega-hawlucha-ex';
import { MegaScraftyex } from './mega-scrafty-ex';
import { Misdreavus } from './misdreavus';
import { Mismagius } from './mismagius';
import { NightMine } from './night-mine';
import { Numel } from './numel';
import { NsZekrom } from './ns-zekrom';
import { NsVanillish } from './ns-vanillish';
import { NsVanillite } from './ns-vanillite';
import { NsVanilluxe } from './ns-vanilluxe';
import { LightBall } from './light-ball';
import { PokePad } from './poke-pad';
import { Rayquaza } from './rayquaza';
import { Rotom } from './rotom';
import { Scraggy } from './scraggy';
import { Snom } from './snom';
import { Snorunt } from './snorunt';
import { TapuKoko } from './tapu-koko';
import { TeamRocketsHonchkrow } from './team-rockets-honchkrow';
import { Wurmple } from './wurmple';
import { Silcoon } from './silcoon';
// Start Deck 100
import { ErikasOddish } from './erikas-oddish';
import { ErikasGloom } from './erikas-gloom';
import { ErikasVileplumeex } from './erikas-vileplume-ex';
import { ErikasBellsprout } from './erikas-bellsprout';
import { ErikasWeepinbell } from './erikas-weepinbell';
import { ErikasVictreebel } from './erikas-victreebel';
import { ErikasTangela } from './erikas-tangela';
import { Chikorita } from './chikorita';
import { Bayleef } from './bayleef';
import { MegaMeganiumex } from './mega-meganium-ex';
import { Charmander } from './charmander';
import { Charmeleon } from './charmeleon';
import { Tepig } from './tepig';
import { Pignite } from './pignite';
import { MegaEmboarex } from './mega-emboar-ex';
import { Totodile } from './totodile';
import { Croconaw } from './croconaw';
import { MegaFeraligatrex } from './mega-feraligatr-ex';
import { Regiceex } from './regice-ex';
import { Marill } from './marill';
import { Azumarillex } from './azumarill-ex';
import { Registeelex } from './registeel-ex';
import { LarrysDunsparce } from './larrys-dunsparce';
import { LarrysDudunsparceex } from './larrys-dudunsparce-ex';
import { LarrysStarly } from './larrys-starly';
import { LarrysStaravia } from './larrys-staravia';
import { LarrysStaraptor } from './larrys-staraptor';
import { MegaAudinoex } from './mega-audino-ex';
import { LarrysRufflet } from './larrys-rufflet';
import { LarrysBraviary } from './larrys-braviary';
import { LarrysKomala } from './larrys-komala';
import { Waitress } from './waitress';
import { BossOrdersASC } from './other-prints';

export const setAscendedHeroes: Card[] = [
  new AntheaAndConcordia(),
  new Beautifly(),
  new BossOrdersASC(),
  new Cascoon(),
  new Camerupt(),
  new Canari(),
  new Dragonair(),
  new Dratini(),
  new Dustox(),
  new Frosmoth(),
  new LightBall(),
  new MegaCharizardYex(),
  new MegaDragoniteex(),
  new MegaEelektrossex(),
  new MegaFroslassex(),
  new MegaHawluchaex(),
  new MegaScraftyex(),
  new Misdreavus(),
  new Mismagius(),
  new NightMine(),
  new Numel(),
  new NsZekrom(),
  new NsVanillish(),
  new NsVanillite(),
  new NsVanilluxe(),
  new PokePad(),
  new Rayquaza(),
  new Rotom(),
  new Scraggy(),
  new Silcoon(),
  new Snom(),
  new Snorunt(),
  new TapuKoko(),
  new TeamRocketsHonchkrow(),
  new Wurmple(),
  // Start Deck 100
  new ErikasOddish(),
  new ErikasGloom(),
  new ErikasVileplumeex(),
  new ErikasBellsprout(),
  new ErikasWeepinbell(),
  new ErikasVictreebel(),
  new ErikasTangela(),
  new Chikorita(),
  new Bayleef(),
  new MegaMeganiumex(),
  new Charmander(),
  new Charmeleon(),
  new Tepig(),
  new Pignite(),
  new MegaEmboarex(),
  new Totodile(),
  new Croconaw(),
  new MegaFeraligatrex(),
  new Regiceex(),
  new Marill(),
  new Azumarillex(),
  new Registeelex(),
  new LarrysDunsparce(),
  new LarrysDudunsparceex(),
  new LarrysStarly(),
  new LarrysStaravia(),
  new LarrysStaraptor(),
  new MegaAudinoex(),
  new LarrysRufflet(),
  new LarrysBraviary(),
  new LarrysKomala(),
  new Waitress(),
  new BudewASC(),
  new TeamRocketsTarountulaASC(),
  new TeamRocketsSpidopsASC(),
  new EthansSlugmaASC(),
  new EthansMagcargoASC(),
  new EthansHoOhexASC(),
  new NsDarumakaASC(),
  new NsDarmanitanASC(),
  new SalanditASC(),
  new SalazzleASC(),
  new ScorbunnyASC(),
  new RabootASC(),
  new CinderaceexASC(),
  new PsyduckASC(),
  new GolduckASC(),
  new SneaselASC(),
  new WeavileASC(),
  new PikachuexASC(),
  new TynamoASC(),
  new EelektrikASC(),
  new CharjabugASC(),
  new IonosTadbulbASC(),
  new IonosBelliboltexASC(),
  new IonosWattrelASC(),
  new IonosKilowattrelASC(),
  new ClefairyASC(),
  new ClefableASC(),
  new LilliesClefairyexASC(),
  new TeamRocketsMewtwoexASC(),
  new TogepiASC(),
  new TogeticASC(),
  new TogekissASC(),
  new RaltsASC(),
  new KirliaASC(),
  new MegaGardevoirexASC(),
  new ShuppetASC(),
  new BanetteASC(),
  new TeamRocketsMimikyuASC(),
  new MunkidoriASC(),
  new LunatoneASC(),
  new SolrockASC(),
  new RegirockexASC(),
  new CynthiasGibleASC(),
  new CynthiasGabiteASC(),
  new CynthiasGarchompexASC(),
  new RioluASC(),
  new MegaLucarioexASC(),
  new GastlyASC(),
  new HaunterASC(),
  new MegaGengarexASC(),
  new TeamRocketsMurkrowASC(),
  new CynthiasSpiritombASC(),
  new NsZoruaASC(),
  new NsZoroarkexASC(),
  new FezandipitiexASC(),
  new NsReshiramASC(),
  new DreepyASC(),
  new DrakloakASC(),
  new DragapultexASC(),
  new TeamRocketsMeowthASC(),
  new FanRotomASC(),
  new HopsCramorantASC(),
  new TerapagosASC(),
  new TerapagosexASC(),
  new AirBalloonASC(),
  new BuddyBuddyPoffinASC(),
  new CounterGainASC(),
  new LushForestASC(),
  new GlassTrumpetASC(),
  new IrisFightingSpiritASC(),
  new LilliesDeterminationASC(),
  new MegaSignalASC(),
  new MysteryGardenASC(),
  new NsPPUpASC(),
  new NightlyStretcherASC(),
  new PowerProteinASC(),
  new SurferASC(),
  new TeamRocketsArcherASC(),
  new TeamRocketsArianaASC(),
  new TeamRocketsFactoryASC(),
  new TeamRocketsGiovanniASC(),
  new TeamRocketsGreatBallASC(),
  new TeamRocketsPetrelASC(),
  new TeamRocketsProtonASC(),
  new TeamRocketsTransceiverASC(),
  new TeamRocketsWatchtowerASC(),
  new ToolScrapperASC(),
  new UltraBallASC(),
  new PrismEnergyASC(),
  new TeamRocketsEnergyASC(),
  new ErikasTangela2ASC(),
  new Beautifly2ASC(),
  new Dustox2ASC(),
  new Budew2ASC(),
  new EthansMagcargo2ASC(),
  new Numel2ASC(),
  new Salazzle2ASC(),
  new Scorbunny2ASC(),
  new Psyduck2ASC(),
  new Snorunt2ASC(),
  new Weavile2ASC(),
  new IonosWattrel2ASC(),
  new Marill2ASC(),
  new Misdreavus2ASC(),
  new Banette2ASC(),
  new Togekiss2ASC(),
  new TeamRocketsMimikyu2ASC(),
  new CynthiasSpiritomb2ASC(),
  new Dreepy2ASC(),
  new Drakloak2ASC(),
  new LarrysStaraptor2ASC(),
  new FanRotom2ASC(),
  new MegaAudinoex2ASC(),
  new BlackBeltsTrainingASC(),
  new BossOrders2ASC(),
  new Canari2ASC(),
  new CherenASC(),
  new CounterGain2ASC(),
  new GlassTrumpet2ASC(),
  new JammingTowerASC(),
  new NsPPUp2ASC(),
  new TeamRocketsTransceiver2ASC(),
  new UltraBall2ASC(),
  new MegaFroslassex2ASC(),
  new MegaEelektrossex2ASC(),
  new MegaDiancieexASC(),
  new MegaHawluchaex2ASC(),
  new MegaGengarex2ASC(),
  new MegaScraftyex2ASC(),
  new MegaDragoniteex2ASC(),
  new MegaMeganiumex2ASC(),
  new MegaEmboarex2ASC(),
  new MegaFeraligatrex2ASC(),
  new MegaFroslassex3ASC(),
  new Pikachuex3ASC(),
  new MegaEelektrossex3ASC(),
  new IonosBelliboltex2ASC(),
  new LilliesClefairyex2ASC(),
  new TeamRocketsMewtwoex2ASC(),
  new MegaDiancieex2ASC(),
  new MegaHawluchaex3ASC(),
  new MegaGengarex3ASC(),
  new MegaScraftyex3ASC(),
  new NsZoroarkex2ASC(),
  new MarniesGrimmsnarlexASC(),
  new Fezandipitiex2ASC(),
  new StevensMetagrossexASC(),
  new MegaDragoniteex3ASC(),
  new Canari3ASC(),
  new IrisFightingSpirit2ASC(),
  new Surfer2ASC(),
  new MegaCharizardYex2ASC(),
  new MegaDragoniteex4ASC(),
];