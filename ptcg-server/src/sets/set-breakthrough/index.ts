import { Card } from '../../game/store/card/card';
import { Abomasnow } from './abomasnow';
import { Aromatisse } from './aromatisse';
import { Axew } from './axew';
import { Axew2 } from './axew-2';
import { Beheeyem } from './beheeyem';
import { Braixen } from './braixen';
import { Braviary } from './braviary';
import { Bronzong } from './bronzong';
import { Bronzor } from './bronzor';
import { Cacnea } from './cacnea';
import { Cacturne } from './cacturne';
import { Chatot } from './chatot';
import { Chesnaught } from './chesnaught';
import { ChesnaughtBreak } from './chesnaught-break';
import { Chespin } from './chespin';
import { Chespin2 } from './chespin-2';
import { Chespin3 } from './chespin-3';
import { Cresselia } from './cresselia';
import { Cubone } from './cubone';
import { Cyndaquil } from './cyndaquil';
import { Dedenne } from './dedenne';
import { Dodrio } from './dodrio';
import { Doduo } from './doduo';
import { Doduo2 } from './doduo-2';
import { Elgyem } from './elgyem';
import { Empoleon } from './empoleon';
import { Fennekin } from './fennekin';
import { Flabebe } from './flabebe';
import { Floette } from './floette';
import { Florges } from './florges';
import { FlorgesBreak } from './florges-break';
import { Fraxure } from './fraxure';
import { Froakie } from './froakie';
import { Frogadier } from './frogadier';
import { Gallade } from './gallade';
import { Gastly } from './gastly';
import { Gengar } from './gengar';
import { GlalieEx } from './glalie-ex';
import { Gogoat } from './gogoat';
import { Goldeen } from './goldeen';
import { Granbull } from './granbull';
import { Haunter } from './haunter';
import { Hawlucha } from './hawlucha';
import { Haxorus } from './haxorus';
import { Hippopotas } from './hippopotas';
import { Hoothoot } from './hoothoot';
import { HoundoomEx } from './houndoom-ex';
import { Inkay } from './inkay';
import { Kirlia } from './kirlia';
import { MGlalieEx } from './m-glalie-ex';
import { MHoundoomEx } from './m-houndoom-ex';
import { MMewtwoEx } from './m-mewtwo-ex';
import { MMewtwoEx2 } from './m-mewtwo-ex-2';
import { Magnemite } from './magnemite';
import { Magnemite2 } from './magnemite-2';
import { Magneton } from './magneton';
import { Magnezone } from './magnezone';
import { Mamoswine } from './mamoswine';
import { Marowak } from './marowak';
import { MarowakBreak } from './marowak-break';
import { Meloetta } from './meloetta';
import { Meowth } from './meowth';
import { MewtwoEx } from './mewtwo-ex';
import { MewtwoEx2 } from './mewtwo-ex-2';
import { Misdreavus } from './misdreavus';
import { Mismagius } from './mismagius';
import { MrMime } from './mr-mime';
import { Noctowl } from './noctowl';
import { Noibat } from './noibat';
import { Noibat2 } from './noibat-2';
import { Noivern } from './noivern';
import { NoivernBreak } from './noivern-break';
import { Octillery } from './octillery';
import { Pancham } from './pancham';
import { Panpour } from './panpour';
import { Pansage } from './pansage';
import { Pansear } from './pansear';
import { Paras } from './paras';
import { Parasect } from './parasect';
import { Pikachu } from './pikachu';
import { Piloswine } from './piloswine';
import { Pinsir } from './pinsir';
import { Piplup } from './piplup';
import { Prinplup } from './prinplup';
import { Quilava } from './quilava';
import { Quilladin } from './quilladin';
import { Raichu } from './raichu';
import { RaichuBreak } from './raichu-break';
import { Raikou } from './raikou';
import { Ralts } from './ralts';
import { Ralts2 } from './ralts-2';
import { Remoraid } from './remoraid';
import { Remoraid2 } from './remoraid-2';
import { Rufflet } from './rufflet';
import { Sandshrew } from './sandshrew';
import { Sandslash } from './sandslash';
import { Scatterbug } from './scatterbug';
import { Seaking } from './seaking';
import { Simipour } from './simipour';
import { Simisage } from './simisage';
import { Simisear } from './simisear';
import { Skiddo } from './skiddo';
import { Smeargle } from './smeargle';
import { Snorlax } from './snorlax';
import { Snover } from './snover';
import { Snubbull } from './snubbull';
import { Spewpa } from './spewpa';
import { Spritzee } from './spritzee';
import { Staraptor } from './staraptor';
import { Staravia } from './staravia';
import { Starly } from './starly';
import { Starmie } from './starmie';
import { Staryu } from './staryu';
import { Stunfisk } from './stunfisk';
import { Swinub } from './swinub';
import { Swoobat } from './swoobat';
import { Teddiursa } from './teddiursa';
import { Typhlosion } from './typhlosion';
import { Ursaring } from './ursaring';
import { Vanillish } from './vanillish';
import { Vanillite } from './vanillite';
import { Vanilluxe } from './vanilluxe';
import { Vivillon } from './vivillon';
import { Wobbuffet } from './wobbuffet';
import { Woobat } from './woobat';
import { Xerneas } from './xerneas';
import { Yveltal } from './yveltal';
import { ZoroarkBreak } from './zoroark-break';
import { Zorua } from './zorua';
import { Zorua2 } from './zorua-2';
import { AssaultVest } from './assault-vest';
import { Brigette } from './brigette';
import { BuddyBuddyRescue } from './buddy-buddy-rescue';
import { GiovannisScheme } from './giovannis-scheme';
import { GlalieSpiritLink } from './glalie-spirit-link';
import { HeavyBoots } from './heavy-boots';
import { HoundoomSpiritLink } from './houndoom-spirit-link';
import { MewtwoSpiritLink } from './mewtwo-spirit-link';
import { ParallelCity } from './parallel-city';
import { ReservedTicket } from './reserved-ticket';
import { TownMap } from './town-map';
import { BurningEnergy } from './burning-energy';

import {
  FishermanBKT,
  FloatStoneBKT,
  ProfessorsLetterBKT,
  HeavyBallBKT,
  JudgeULBKT,
  SkylaBKT,
  SuperRodBKT,
  RainbowEnergyPLBKT,
  Brigette2BKT,
  ProfessorsLetter2BKT,
  SwabluBKT,
  HoundoomEx2,
  MHoundoomEx2,
  GlalieEx2,
  MGlalieEx2,
  MewtwoEx3,
  MewtwoEx4,
  MMewtwoEx3,
  MMewtwoEx4,
  GiovannisScheme2,
  MewtwoEx5,
  MewtwoEx6,
} from './other-prints';

export const setBreakthrough: Card[] = [
  // Pokemon
  new Abomasnow(),
  new Aromatisse(),
  new Axew(),
  new Axew2(),
  new Beheeyem(),
  new Braixen(),
  new Braviary(),
  new Bronzong(),
  new Bronzor(),
  new Cacnea(),
  new Cacturne(),
  new Chatot(),
  new Chesnaught(),
  new ChesnaughtBreak(),
  new Chespin(),
  new Chespin2(),
  new Chespin3(),
  new Cresselia(),
  new Cubone(),
  new Cyndaquil(),
  new Dedenne(),
  new Dodrio(),
  new Doduo(),
  new Doduo2(),
  new Elgyem(),
  new Empoleon(),
  new Fennekin(),
  new Flabebe(),
  new Floette(),
  new Florges(),
  new FlorgesBreak(),
  new Fraxure(),
  new Froakie(),
  new Frogadier(),
  new Gallade(),
  new Gastly(),
  new Gengar(),
  new GlalieEx(),
  new Gogoat(),
  new Goldeen(),
  new Granbull(),
  new Haunter(),
  new Hawlucha(),
  new Haxorus(),
  new Hippopotas(),
  new Hoothoot(),
  new HoundoomEx(),
  new Inkay(),
  new Kirlia(),
  new MGlalieEx(),
  new MHoundoomEx(),
  new MMewtwoEx(),
  new MMewtwoEx2(),
  new Magnemite(),
  new Magnemite2(),
  new Magneton(),
  new Magnezone(),
  new Mamoswine(),
  new Marowak(),
  new MarowakBreak(),
  new Meloetta(),
  new Meowth(),
  new MewtwoEx(),
  new MewtwoEx2(),
  new Misdreavus(),
  new Mismagius(),
  new MrMime(),
  new Noctowl(),
  new Noibat(),
  new Noibat2(),
  new Noivern(),
  new NoivernBreak(),
  new Octillery(),
  new Pancham(),
  new Panpour(),
  new Pansage(),
  new Pansear(),
  new Paras(),
  new Parasect(),
  new Pikachu(),
  new Piloswine(),
  new Pinsir(),
  new Piplup(),
  new Prinplup(),
  new Quilava(),
  new Quilladin(),
  new Raichu(),
  new RaichuBreak(),
  new Raikou(),
  new Ralts(),
  new Ralts2(),
  new Remoraid(),
  new Remoraid2(),
  new Rufflet(),
  new Sandshrew(),
  new Sandslash(),
  new Scatterbug(),
  new Seaking(),
  new Simipour(),
  new Simisage(),
  new Simisear(),
  new Skiddo(),
  new Smeargle(),
  new Snorlax(),
  new Snover(),
  new Snubbull(),
  new Spewpa(),
  new Spritzee(),
  new Staraptor(),
  new Staravia(),
  new Starly(),
  new Starmie(),
  new Staryu(),
  new Stunfisk(),
  new Swinub(),
  new Swoobat(),
  new Teddiursa(),
  new Typhlosion(),
  new Ursaring(),
  new Vanillish(),
  new Vanillite(),
  new Vanilluxe(),
  new Vivillon(),
  new Wobbuffet(),
  new Woobat(),
  new Xerneas(),
  new Yveltal(),
  new ZoroarkBreak(),
  new Zorua(),
  new Zorua2(),

  // Trainers
  new AssaultVest(),
  new Brigette(),
  new BuddyBuddyRescue(),
  new GiovannisScheme(),
  new GlalieSpiritLink(),
  new HeavyBoots(),
  new HoundoomSpiritLink(),
  new MewtwoSpiritLink(),
  new ParallelCity(),
  new ReservedTicket(),
  new TownMap(),

  // Energy
  new BurningEnergy(),

  // Other Prints (Reprints & Alt Arts)
  new FishermanBKT(),
  new FloatStoneBKT(),
  new ProfessorsLetterBKT(),
  new HeavyBallBKT(),
  new JudgeULBKT(),
  new SkylaBKT(),
  new SuperRodBKT(),
  new RainbowEnergyPLBKT(),
  new Brigette2BKT(),
  new ProfessorsLetter2BKT(),
  new SwabluBKT(),
  new HoundoomEx2(),
  new MHoundoomEx2(),
  new GlalieEx2(),
  new MGlalieEx2(),
  new MewtwoEx3(),
  new MewtwoEx4(),
  new MMewtwoEx3(),
  new MMewtwoEx4(),
  new GiovannisScheme2(),
  new MewtwoEx5(),
  new MewtwoEx6(),
];
