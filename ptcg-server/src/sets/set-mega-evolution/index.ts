import { BossOrdersMEG, EnergySwitchPKMEG, RareCandyMEG, SwitchMEG, UltraBallMEG, Bulbasaur2MEG, Ivysaur2MEG, Ninjask2MEG, Snover2MEG, Clawitizer2MEG, Inteleon2MEG, Houndstone2MEG, Garganacl2MEG, Steelix2MEG, Gumshoos2MEG, MegaVenusaurEx2MEG, MegaCameruptEx2MEG, MegaAbomasnowEx2MEG, MegaManectricEx2MEG, MegaGardevoirex2MEG, MegaLucarioex2MEG, MegaAbsolex2MEG, MegaMawileEx2MEG, MegaLatiasex2MEG, MegaKangaskhanex2MEG, AirBalloonMEG, BuddyBuddyPoffinMEG, FightingGong2MEG, LilliesDetermination2MEG, MegaSignal2MEG, MysteryGarden2MEG, NightlyStretcherMEG, PowerProtein2MEG, RareCandy2MEG, WallysCompassion2MEG, MegaVenusaurEx3MEG, MegaGardevoirex3MEG, MegaLucarioex3MEG, MegaAbsolex3MEG, MegaLatiasex3MEG, MegaKangaskhanex3MEG, LilliesDetermination3MEG, WallysCompassion3MEG, MegaGardevoirex4MEG, MegaLucarioex4MEG } from './other-prints';
import { Card } from '../../game/store/card/card';
import { Abra } from './abra';
import { Alakazam } from './alakazam';
import { Bayleef } from './bayleef';
import { Bulbasaur } from './bulbasaur';
import { Buneary } from './buneary';
import { Celebi } from './celebi';
import { Chikorita } from './chikorita';
import { Cinderace } from './cinderace';
import { Clauncher } from './clauncher';
import { Corphish } from './corphish';
import { Crawdaunt } from './crawdaunt';
import { DangerousRuins } from './risky-ruins';
import { Drizzile } from './drizzile';
import { Delibird } from './delibird';
import { Electrike } from './electrike';
import { FightingGong } from './fighting-gong';
import { Garganacl } from './garganacl';
import { Gimmighoul } from './gimmighoul';
import { Greavard } from './greavard';
import { Gumshoos } from './gumshoos';
import { Hariyama } from './hariyama';
import { Houndstone } from './houndstone';
import { Ivysaur } from './ivysaur';
import { Kadabra } from './kadabra';
import { Kirlia } from './kirlia';
import { Kyogre } from './kyogre';
import { Latios } from './latios';
import { LilliesDetermination } from './lillies-determination';
import { Lunatone } from './lunatone';
import { LushForest } from './forest-of-vitality';
import { Magnemite } from './magnemite';
import { Makuhita } from './makuhita';
import { MegaAbomasnowEx } from './mega-abomasnow-ex';
import { MegaAbsolex } from './mega-absol-ex';
import { MegaCameruptEx } from './mega-camerupt-ex';
import { MegaGardevoirex } from './mega-gardevoir-ex';
import { MegaKangaskhanex } from './mega-kangaskhan-ex';
import { MegaLatiasex } from './mega-latias-ex';
import { MegaLucarioex } from './mega-lucario-ex';
import { MegaManectricEx } from './mega-manectric-ex';
import { MegaMawileEx } from './mega-mawile-ex';
import { Meganium } from './meganium';
import { MegaSignal } from './mega-signal';
import { MegaVenusaurEx } from './mega-venusaur-ex';
import { MysteryGarden } from './mystery-garden';
import { Nacli } from './nacli';
import { Naclstack } from './naclstack';
import { Nincada } from './nincada';
import { Ninjask } from './ninjask';
import { Numel } from './numel';
import { Onix } from './onix';
import { PowerProtein } from './power-protein';
import { Ralts } from './ralts';
import { Repel } from './repel';
import { Riolu } from './riolu';
import { Snover } from './snover';
import { Sobble } from './sobble';
import { Solrock } from './solrock';
import { Steelix } from './steelix';
import { SurfingBeach } from './surfing-beach';
import { WallysCompassion } from './wallys-compassion';
import { Xerneas } from './xerneas';
import { Yungoos } from './yungoos';
import { Yveltal } from './yveltal';

export const setMegaEvolution: Card[] = [
  new Abra(),
  new Alakazam(),
  new Bayleef(),
  new Bulbasaur(),
  new Buneary(),
  new Celebi(),
  new Chikorita(),
  new Cinderace(),
  new Clauncher(),
  new Corphish(),
  new Crawdaunt(),
  new DangerousRuins(),
  new Delibird(),
  new Drizzile(),
  new Electrike(),
  new FightingGong(),
  new Garganacl(),
  new Gimmighoul(),
  new Greavard(),
  new Gumshoos(),
  new Hariyama(),
  new Houndstone(),
  new Ivysaur(),
  new Kadabra(),
  new Kirlia(),
  new Kyogre(),
  new Latios(),
  new LilliesDetermination(),
  new Lunatone(),
  new LushForest(),
  new Magnemite(),
  new Makuhita(),
  new MegaAbomasnowEx(),
  new MegaAbsolex(),
  new MegaCameruptEx(),
  new MegaGardevoirex(),
  new MegaKangaskhanex(),
  new MegaLatiasex(),
  new MegaLucarioex(),
  new MegaManectricEx(),
  new MegaMawileEx(),
  new Meganium(),
  new MegaSignal(),
  new MegaVenusaurEx(),
  new MysteryGarden(),
  new Nacli(),
  new Naclstack(),
  new Nincada(),
  new Ninjask(),
  new Numel(),
  new Onix(),
  new PowerProtein(),
  new Ralts(),
  new Repel(),
  new Riolu(),
  new Snover(),
  new Sobble(),
  new Solrock(),
  new Steelix(),
  new SurfingBeach(),
  new WallysCompassion(),
  new Xerneas(),
  new Yungoos(),
  new Yveltal(),

  // Other Prints
  // new BulbasaurM1S(),
  // new InteleonM1S(),
  // new MegaGardevoirexM1S(),
  // new MegaKangaskhanexM1S(),
  // new MegaLucarioexM1S(),
  // new MegaVenusaurExM1S(),
  // new NightStretcherM1S(),
  // new RareCandyM1S(),
  new BossOrdersMEG(),
  new EnergySwitchPKMEG(),
  new RareCandyMEG(),
  new SwitchMEG(),
  new UltraBallMEG(),
  new Bulbasaur2MEG(),
  new Ivysaur2MEG(),
  new Ninjask2MEG(),
  new Snover2MEG(),
  new Clawitizer2MEG(),
  new Inteleon2MEG(),
  new Houndstone2MEG(),
  new Garganacl2MEG(),
  new Steelix2MEG(),
  new Gumshoos2MEG(),
  new MegaVenusaurEx2MEG(),
  new MegaCameruptEx2MEG(),
  new MegaAbomasnowEx2MEG(),
  new MegaManectricEx2MEG(),
  new MegaGardevoirex2MEG(),
  new MegaLucarioex2MEG(),
  new MegaAbsolex2MEG(),
  new MegaMawileEx2MEG(),
  new MegaLatiasex2MEG(),
  new MegaKangaskhanex2MEG(),
  new AirBalloonMEG(),
  new BuddyBuddyPoffinMEG(),
  new FightingGong2MEG(),
  new LilliesDetermination2MEG(),
  new MegaSignal2MEG(),
  new MysteryGarden2MEG(),
  new NightlyStretcherMEG(),
  new PowerProtein2MEG(),
  new RareCandy2MEG(),
  new WallysCompassion2MEG(),
  new MegaVenusaurEx3MEG(),
  new MegaGardevoirex3MEG(),
  new MegaLucarioex3MEG(),
  new MegaAbsolex3MEG(),
  new MegaLatiasex3MEG(),
  new MegaKangaskhanex3MEG(),
  new LilliesDetermination3MEG(),
  new WallysCompassion3MEG(),
  new MegaGardevoirex4MEG(),
  new MegaLucarioex4MEG(),
];
