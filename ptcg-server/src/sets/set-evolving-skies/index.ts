import { Card } from '../../game/store/card/card';
import { Altaria } from './altaria';
import { Ampharos } from './ampharos';
import { Appletun } from './appletun';
import { Applin } from './applin';
import { ArctovishV } from './arctovish-v';
import { Avalugg } from './avalugg';
import { Bagon } from './bagon';
import { Bergmite } from './bergmite';
import { Boldore } from './boldore';
import { Braviary } from './braviary';
import { Carvanha } from './carvanha';
import { Chinchou } from './chinchou';
import { Crustle } from './crustle';
import { Cryogonal } from './cryogonal';
import { Cutiefly } from './cutiefly';
import { Deino } from './deino';
import { Dialga } from './dialga';
import { DracozoltV } from './dracozolt-v';
import { DracozoltVmax } from './dracozolt-vmax';
import { DragoniteV } from './dragonite-v';
import { Drampa } from './drampa';
import { Drowzee } from './drowzee';
import { DuraludonV } from './duraludon-v';
import { DuraludonVMAX } from './duraludon-vmax';
import { Dwebble } from './dwebble';
import { Eevee } from './eevee';
import { Eiscue } from './eiscue';
import { Eldegoss } from './eldegoss';
import { Emolga } from './emolga';
import { Entei } from './entei';
import { EspeonV } from './espeon-v';
import { EspeonVMAX } from './espeon-vmax';
import { Feebas } from './feebas';
import { Flaaffy } from './flaaffy';
import { Flabebe } from './flabebe';
import { Flapple } from './flapple';
import { FlareonV } from './flareon-v';
import { FlareonVmax } from './flareon-vmax';
import { Fletchinder } from './fletchinder';
import { Fletchling } from './fletchling';
import { Floette } from './floette';
import { Florges } from './florges';
import { Froslass } from './froslass';
import { RayquazaVAA } from './full-art';
import { GalarianZapdos } from './galarian_zapdos';
import { GalarianArticuno } from './galarian-articuno';
import { GalarianMoltres } from './galarian-moltres';
import { GarbodorV } from './garbodor-v';
import { GarbodorVMAX } from './garbodor-vmax';
import { Gigalith } from './gigalith';
import { GlaceonV } from './glaceon-v';
import { GlaceonVmax } from './glaceon-vmax';
import { Golduck } from './golduck';
import { GolurkV } from './golurk-v';
import { Gossifleur } from './gossifleur';
import { Gourgeist } from './gourgeist';
import { GyaradosV } from './gyarados-v';
import { GyaradosVMAX } from './gyarados-vmax';
import { Herdier } from './herdier';
import { Hippopotas } from './hippopotas';
import { Hippowdon } from './hippowdon';
import { Hitmonchan } from './hitmonchan';
import { Hoppip } from './hoppip';
import { Hydreigon } from './hydreigon';
import { Hypno } from './hypno';
import { Inteleon } from './inteleon';
import { JolteonV } from './jolteon-v';
import { JolteonVmax } from './jolteon-vmax';
import { Jumpluff } from './jumpluff';
import { Kyurem } from './kyurem';
import { Lanturn } from './lanturn';
import { LeafeonV } from './leafeon-v';
import { LeafeonVMAX } from './leafeon-vmax';
import { Lilligant } from './lilligant';
import { Lillipup } from './lillipup';
import { Litleo } from './litleo';
import { Lombre } from './lombre';
import { Lotad } from './lotad';
import { Ludicolo } from './ludicolo';
import { Luvdisc } from './luvdisc';
import { LycanrocV } from './lycanroc-v';
import { LycanrocVmax } from './lycanroc-vmax';
import { Mareep } from './mareep';
import { Marshadow } from './marshadow';
import { MedichamV } from './medicham-v';
import { Milotic } from './milotic';
import { Nickit } from './nickit';
import { NoivernV } from './noivern-v';
import { Nuzleaf } from './nuzleaf';
import { Palpitoad } from './palpitoad';
import { Petilil } from './petilil';
import { Pikachu } from './pikachu';
import { Pinsir } from './pinsir';
import { Psyduck } from './psyduck';
import { Pumpkaboo } from './pumpkaboo';
import { Pyroar } from './pyroar';
import { Raichu } from './raichu';
import { RayquazaV } from './rayquaza-v';
import { RayquazaVMAX } from './rayquaza-vmax';
import { Regidrago } from './regidrago';
import { Regieleki } from './regieleki';
import { Ribombee } from './ribombee';
import { Roggenrola } from './roggenrola';
import { Rufflet } from './rufflet';
import { Sableye } from './sableye';
import { Salamence } from './salamence';
import { Scrafty } from './scrafty';
import { Scraggy } from './scraggy';
import { Seedot } from './seedot';
import { Seismitoad } from './seismitoad';
import { Sharpedo } from './sharpedo';
import { Shelgon } from './shelgon';
import { Shiftry } from './shiftry';
import { Skiploom } from './skiploom';
import { Slaking } from './slaking';
import { Smeargle } from './smeargle';
import { Stoutland } from './stoutland';
import { SuicuneV } from './suicune-v';
import { Swablu } from './swablu';
import { Swoobat } from './swoobat';
import { SylveonV } from './sylveon-v';
import { SylveonVMAX } from './sylveon-vmax';
import { Talonflame } from './talonflame';
import { Teddiursa } from './teddiursa';
import { Tentacool } from './tentacool';
import { Tentacruel } from './tentacruel';
import { Thievul } from './thievul';
import { TrevenantV } from './trevenant-v';
import { TrevenantVmax } from './trevenant-vmax';
import { Tropius } from './tropius';
import { Tympole } from './tympole';
import { UmbreonV } from './umbreon-v';
import { UmbreonVMAX } from './umbreon-vmax';
import { Ursaring } from './ursaring';
import { VaporeonV } from './vaporeon-v';
import { VaporeonVmax } from './vaporeon-vmax';
import { Victini } from './victini';
import { Vigoroth } from './vigoroth';
import { VolcaronaV } from './volcarona-v';
import { Wishiwashi } from './wishiwashi';
import { Wobbuffet } from './wobbuffet';
import { Woobat } from './woobat';
import { Zoroark } from './zoroark';
import { Zorua } from './zorua';
import { Zweilous } from './zweilous';
import { Zygarde } from './zygarde';
import { AromaLady } from './aroma-lady';
import { BoostShake } from './boost-shake';
import { Copycat } from './copycat';
import { CrystalCave } from './crystal-cave';
import { DiggingGloves } from './digging-gloves';
import { DreamBall } from './dream-ball';
import { ElementalBadge } from './elemental-badge';
import { FullFaceGuard } from './full-face-guard';
import { Gordie } from './gordie';
import { LuckyIcePop } from './lucky-ice-pop';
import { MoonAndSunBadge } from './moon-and-sun-badge';
import { Raihan } from './raihan';
import { RapidStrikeScrollOfTheFlyingDragon } from './rapid-strike-scroll-of-the-flying-dragon';
import { RescueCarrier } from './rescue-carrier';
import { RibbonBadge } from './ribbon-badge';
import { RubberGloves } from './rubber-gloves';
import { ShoppingCenter } from './shopping-center';
import { SingleStrikeScrollOfTheFangedDragon } from './single-strike-scroll-of-the-fanged-dragon';
import { SnowLeafBadge } from './snow-leaf-badge';
import { SpiritMask } from './spirit-mask';
import { StormyMountains } from './stormy-mountains';
import { SwitchingCups } from './switching-cups';
import { ToyCatcher } from './toy-catcher';
import { ZinniasResolve } from './zinnias-resolve';
import { TreasureEnergy } from './treasure-energy';

import {
  SlakothEVS,
  LeafeonV2,
  LeafeonV3,
  TrevenantV2,
  VolcaronaV2,
  GyaradosV2,
  SuicuneV2,
  GlaceonV2,
  GlaceonV3,
  ArctovishV2,
  DracozoltV2,
  EspeonV2,
  EspeonV3,
  GolurkV2,
  GolurkV3,
  SylveonV2,
  SylveonV3,
  MedichamV2,
  MedichamV3,
  LycanrocV2,
  UmbreonV2,
  UmbreonV3,
  GarbodorV2,
  DragoniteV2,
  RayquazaV2,
  NoivernV2,
  NoivernV3,
  DuraludonV2,
  DuraludonV3,
  AromaLady2,
  Copycat2,
  Gordie2,
  Raihan2,
  ZinniasResolve2,
  LeafeonVmax2,
  LeafeonVmax3,
  TrevenantVmax2,
  GyaradosVmax2,
  GlaceonVmax2,
  GlaceonVmax3,
  DracozoltVmax2,
  SylveonVmax2,
  SylveonVmax3,
  LycanrocVmax2,
  UmbreonVmax2,
  UmbreonVmax3,
  GarbodorVMAX2,
  RayquazaVmax2,
  RayquazaVmax3,
  DuraludonVmax2,
  DuraludonVmax3,
  AromaLady3,
  Copycat3,
  Gordie3,
  Raihan3,
  ZinniasResolve3,
  CresseliaEVS,
  BoostShake2,
  CrystalCave2,
  FullFaceGuard2,
  StormyMountains2,
  ToyCatcher2,
  TurffieldStadiumEVS,
} from './other-prints';

export const setEvolvingSkies: Card[] = [
  // Pokemon
  new Altaria(),
  new Ampharos(),
  new Appletun(),
  new Applin(),
  new ArctovishV(),
  new Avalugg(),
  new Bagon(),
  new Bergmite(),
  new Boldore(),
  new Braviary(),
  new Carvanha(),
  new Chinchou(),
  new Crustle(),
  new Cryogonal(),
  new Cutiefly(),
  new Deino(),
  new Dialga(),
  new DracozoltV(),
  new DracozoltVmax(),
  new DragoniteV(),
  new Drampa(),
  new Drowzee(),
  new DuraludonV(),
  new DuraludonVMAX(),
  new Dwebble(),
  new Eevee(),
  new Eiscue(),
  new Eldegoss(),
  new Emolga(),
  new Entei(),
  new EspeonV(),
  new EspeonVMAX(),
  new Feebas(),
  new Flaaffy(),
  new Flabebe(),
  new Flapple(),
  new FlareonV(),
  new FlareonVmax(),
  new Fletchinder(),
  new Fletchling(),
  new Floette(),
  new Florges(),
  new Froslass(),
  new RayquazaVAA(),
  new GalarianZapdos(),
  new GalarianArticuno(),
  new GalarianMoltres(),
  new GarbodorV(),
  new GarbodorVMAX(),
  new Gigalith(),
  new GlaceonV(),
  new GlaceonVmax(),
  new Golduck(),
  new GolurkV(),
  new Gossifleur(),
  new Gourgeist(),
  new GyaradosV(),
  new GyaradosVMAX(),
  new Herdier(),
  new Hippopotas(),
  new Hippowdon(),
  new Hitmonchan(),
  new Hoppip(),
  new Hydreigon(),
  new Hypno(),
  new Inteleon(),
  new JolteonV(),
  new JolteonVmax(),
  new Jumpluff(),
  new Kyurem(),
  new Lanturn(),
  new LeafeonV(),
  new LeafeonVMAX(),
  new Lilligant(),
  new Lillipup(),
  new Litleo(),
  new Lombre(),
  new Lotad(),
  new Ludicolo(),
  new Luvdisc(),
  new LycanrocV(),
  new LycanrocVmax(),
  new Mareep(),
  new Marshadow(),
  new MedichamV(),
  new Milotic(),
  new Nickit(),
  new NoivernV(),
  new Nuzleaf(),
  new Palpitoad(),
  new Petilil(),
  new Pikachu(),
  new Pinsir(),
  new Psyduck(),
  new Pumpkaboo(),
  new Pyroar(),
  new Raichu(),
  new RayquazaV(),
  new RayquazaVMAX(),
  new Regidrago(),
  new Regieleki(),
  new Ribombee(),
  new Roggenrola(),
  new Rufflet(),
  new Sableye(),
  new Salamence(),
  new Scrafty(),
  new Scraggy(),
  new Seedot(),
  new Seismitoad(),
  new Sharpedo(),
  new Shelgon(),
  new Shiftry(),
  new Skiploom(),
  new Slaking(),
  new Smeargle(),
  new Stoutland(),
  new SuicuneV(),
  new Swablu(),
  new Swoobat(),
  new SylveonV(),
  new SylveonVMAX(),
  new Talonflame(),
  new Teddiursa(),
  new Tentacool(),
  new Tentacruel(),
  new Thievul(),
  new TrevenantV(),
  new TrevenantVmax(),
  new Tropius(),
  new Tympole(),
  new UmbreonV(),
  new UmbreonVMAX(),
  new Ursaring(),
  new VaporeonV(),
  new VaporeonVmax(),
  new Victini(),
  new Vigoroth(),
  new VolcaronaV(),
  new Wishiwashi(),
  new Wobbuffet(),
  new Woobat(),
  new Zoroark(),
  new Zorua(),
  new Zweilous(),
  new Zygarde(),

  // Trainers
  new AromaLady(),
  new BoostShake(),
  new Copycat(),
  new CrystalCave(),
  new DiggingGloves(),
  new DreamBall(),
  new ElementalBadge(),
  new FullFaceGuard(),
  new Gordie(),
  new LuckyIcePop(),
  new MoonAndSunBadge(),
  new Raihan(),
  new RapidStrikeScrollOfTheFlyingDragon(),
  new RescueCarrier(),
  new RibbonBadge(),
  new RubberGloves(),
  new ShoppingCenter(),
  new SingleStrikeScrollOfTheFangedDragon(),
  new SnowLeafBadge(),
  new SpiritMask(),
  new StormyMountains(),
  new SwitchingCups(),
  new ToyCatcher(),
  new ZinniasResolve(),

  // Energy
  new TreasureEnergy(),

  // Other Prints (Reprints & Alt Arts)
  new SlakothEVS(),
  new LeafeonV2(),
  new LeafeonV3(),
  new TrevenantV2(),
  new VolcaronaV2(),
  new GyaradosV2(),
  new SuicuneV2(),
  new GlaceonV2(),
  new GlaceonV3(),
  new ArctovishV2(),
  new DracozoltV2(),
  new EspeonV2(),
  new EspeonV3(),
  new GolurkV2(),
  new GolurkV3(),
  new SylveonV2(),
  new SylveonV3(),
  new MedichamV2(),
  new MedichamV3(),
  new LycanrocV2(),
  new UmbreonV2(),
  new UmbreonV3(),
  new GarbodorV2(),
  new DragoniteV2(),
  new RayquazaV2(),
  new NoivernV2(),
  new NoivernV3(),
  new DuraludonV2(),
  new DuraludonV3(),
  new AromaLady2(),
  new Copycat2(),
  new Gordie2(),
  new Raihan2(),
  new ZinniasResolve2(),
  new LeafeonVmax2(),
  new LeafeonVmax3(),
  new TrevenantVmax2(),
  new GyaradosVmax2(),
  new GlaceonVmax2(),
  new GlaceonVmax3(),
  new DracozoltVmax2(),
  new SylveonVmax2(),
  new SylveonVmax3(),
  new LycanrocVmax2(),
  new UmbreonVmax2(),
  new UmbreonVmax3(),
  new GarbodorVMAX2(),
  new RayquazaVmax2(),
  new RayquazaVmax3(),
  new DuraludonVmax2(),
  new DuraludonVmax3(),
  new AromaLady3(),
  new Copycat3(),
  new Gordie3(),
  new Raihan3(),
  new ZinniasResolve3(),
  new CresseliaEVS(),
  new BoostShake2(),
  new CrystalCave2(),
  new FullFaceGuard2(),
  new StormyMountains2(),
  new ToyCatcher2(),
  new TurffieldStadiumEVS(),
];
