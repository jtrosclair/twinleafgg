import { Card } from '../../game/store/card/card';
import { AggronV } from './aggron-v';
import { AggronVMAX } from './aggron-vmax';
import { Alcremie } from './alcremie';
import { ArceusV } from './arceus-v';
import { ArceusVSTAR } from './arceus-vstar';
import { Axew } from './axew';
import { Baltoy } from './baltoy';
import { Beartic } from './beartic';
import { Bibarel } from './bibarel';
import { Bidoof } from './bidoof';
import { Breloom } from './breloom';
import { Buizel } from './buizel';
import { Castform } from './castform';
import { CharizardV } from './charizard-v';
import { CharizardVstar } from './charizard-vstar';
import { Cherubi } from './cherubi';
import { Chimchar } from './chimchar';
import { Chimecho } from './chimecho';
import { Cinccino } from './cinccino';
import { Claydol } from './claydol';
import { Clefable } from './clefable';
import { Clefairy } from './clefairy';
import { Corphish } from './corphish';
import { Crawdaunt } from './crawdaunt';
import { Cubchoo } from './cubchoo';
import { Dedenne } from './dedenne';
import { DracovishV } from './dracovish-v';
import { DrampaV } from './drampa-v';
import { Druddigon } from './druddigon';
import { Dusclops } from './dusclops';
import { Dusknoir } from './dusknoir';
import { Duskull } from './duskull';
import { Eiscue } from './eiscue';
import { Electabuzz } from './electabuzz';
import { Electivire } from './electivire';
import { Empoleon } from './empoleon';
import { EnteiV } from './entei-v';
import { Escavalier } from './escavalier';
import { Exeggcute } from './exeggcute';
import { Exeggutor } from './exeggutor';
import { Farfetchd } from './farfetchd';
import { Floatzel } from './floatzel';
import { Flygon } from './flygon';
import { FlygonV } from './flygon-v';
import { Fraxure } from './fraxure';
import { Gabite } from './gabite';
import { Garchomp } from './garchomp';
import { Gible } from './gible';
import { Golett } from './golett';
import { Golurk } from './golurk';
import { GranbullV } from './granbull-v';
import { Grimer } from './grimer';
import { Grimmsnarl } from './grimmsnarl';
import { Grotle } from './grotle';
import { Hawlucha } from './hawlucha';
import { Haxorus } from './haxorus';
import { Heatran } from './heatran';
import { Hitmontop } from './hitmontop';
import { HonchkrowV } from './honchkrow-v';
import { Impidimp } from './impidimp';
import { Infernape } from './infernape';
import { KinglerV } from './kingler-v';
import { KinglerVmax } from './kingler-vmax';
import { Klang } from './klang';
import { Klink } from './klink';
import { Klinklang } from './klinklang';
import { Lapras } from './lapras';
import { Liepard } from './liepard';
import { Lucario } from './lucario';
import { LumineonV } from './lumineon-v';
import { Luxio } from './luxio';
import { Luxray } from './luxray';
import { Magmar } from './magmar';
import { Magmortar } from './magmortar';
import { Manaphy } from './manaphy';
import { Mewtwo } from './mewtwo';
import { Milcery } from './milcery';
import { MimikyuVmax } from './mimikyu-vmax';
import { Minccino } from './minccino';
import { Moltres } from './moltres';
import { Monferno } from './monferno';
import { Morgrem } from './morgrem';
import { MorpekoV } from './morpeko-v';
import { Mothim } from './mothim';
import { Muk } from './muk';
import { Nosepass } from './nosepass';
import { Pachirisu } from './pachirisu';
import { PikachuV } from './pikachu-v';
import { Piplup } from './piplup';
import { Prinplup } from './prinplup';
import { Probopass } from './probopass';
import { Purrloin } from './purrloin';
import { RaichuV } from './raichu-v';
import { RaikouV } from './raikou-v';
import { Riolu } from './riolu';
import { Sawk } from './sawk';
import { ShayminV } from './shaymin-v';
import { ShayminVSTAR } from './shaymin-vstar';
import { Shinx } from './shinx';
import { Shroomish } from './shroomish';
import { Sigilyph } from './sigilyph';
import { SimisearV } from './simisear-v';
import { Sneasel } from './sneasel';
import { Snorunt } from './snorunt';
import { Spiritomb } from './spiritomb';
import { Staraptor } from './staraptor';
import { Staravia } from './staravia';
import { Starly } from './starly';
import { Starmie } from './starmie';
import { Staryu } from './staryu';
import { Throh } from './throh';
import { Torkoal } from './torkoal';
import { Tornadus } from './tornadus';
import { Torterra } from './torterra';
import { Trapinch } from './trapinch';
import { Tropius } from './tropius';
import { Turtwig } from './turtwig';
import { Vibrava } from './vibrava';
import { Weavile } from './weavile';
import { WhimsicottV } from './whimsicott-v';
import { WhimsicottVSTAR } from './whimsicott-vstar';
import { Wormadam } from './wormadam';
import { Wormadam2 } from './wormadam-2';
import { Wormadam3 } from './wormadam-3';
import { ZarudeV } from './zarude-v';
import { AcerolasPremonition } from './acerolas-premonition';
import { Barry } from './barry';
import { BlunderPolicy } from './blunder-policy';
import { CafeMaster } from './cafe-master';
import { CherensCare } from './cherens-care';
import { CleansingGloves } from './cleansing-gloves';
import { CollapsedStadium } from './collapsed-stadium';
import { CynthiasAmbition } from './cynthias-ambition';
import { FriendsInGalar } from './friends-in-galar';
import { Gloria } from './gloria';
import { HuntingGloves } from './hunting-gloves';
import { Kindler } from './kindler';
import { MagmaBasin } from './magma-basin';
import { MarniesPride } from './marnies-pride';
import { PotHelmet } from './pot-helmet';
import { RoseannesBackup } from './roseannes-backup';
import { TeamYellsCheer } from './team-yells-cheer';
import { DoubleTurboEnergy } from './double-turbo-energy';

import {
  UltraBallHR,
  WhimsicottVFA,
  WhimsicottVSTARRR,
  MimikyuVBRS,
  ZamazentaVBRS,
  BossOrdersBRS,
  ChoiceBeltBRS,
  ProfessorsResearchBRS,
  UltraBallBRS,
  ShayminV2BRS,
  LumineonV2BRS,
  LumineonV3BRS,
  RaichuV2BRS,
  ZamazentaV2BRS,
  ArceusV2BRS,
  ArceusV3BRS,
  CherensCare2BRS,
  CynthiasAmbition2BRS,
  Kindler2BRS,
  MarniesPride2BRS,
  RoseannesBackup2BRS,
  ShayminVSTAR2BRS,
  ArceusVSTAR2BRS,
  CherensCare3BRS,
  CynthiasAmbition3BRS,
  Kindler3BRS,
  RoseannesBackup3BRS,
  GalarianArticunoVBRS,
  GalarianZapdosVBRS,
  GalarianMoltresVBRS,
  ArceusVSTAR3BRS,
  MagmaBasin2BRS,
  GrassEnergyBRS,
  FireEnergyBRS,
  WaterEnergyBRS,
  LightningEnergyBRS,
  PsychicEnergyBRS,
  FightingEnergyBRS,
  DarknessEnergyBRS,
  MetalEnergyBRS,
  OctilleryBRS,
  ZekromBRS,
  Dusknoir2BRS,
  Alcremie2BRS,
  HoundoomBRS,
  OranguruBRS,
  SylveonVBRS,
  SylveonVMAXBRS,
  MimikyuV2BRS,
  SingleStrikeUrshifuVBRS,
  SingleStrikeUrshifuVMAXBRS,
  RapidStrikeUrshifuVBRS,
  RapidStrikeUrshifuVMAXBRS,
  UmbreonVBRS,
  UmbreonVMAXBRS,
  AcerolasPremonition2BRS,
  CafeMaster2BRS,
  Gloria2BRS,
  SingleStrikeStyleMustardBRS,
  SingleStrikeUrshifuVMAX2BRS,
  RapidStrikeUrshifuVMAX2BRS,
  BurmyBRS,
  KarrablastBRS,
  FreshWaterSetBRS,
  CharizardV2,
  CharizardV3,
  GranbullV2,
  HonchkrowV2,
  HonchkrowV3,
  FlygonV2,
  Barry2,
  CharizardVstar2,
} from './other-prints';

export const setBrilliantStars: Card[] = [
  // Pokemon
  new AggronV(),
  new AggronVMAX(),
  new Alcremie(),
  new ArceusV(),
  new ArceusVSTAR(),
  new Axew(),
  new Baltoy(),
  new Beartic(),
  new Bibarel(),
  new Bidoof(),
  new Breloom(),
  new Buizel(),
  new Castform(),
  new CharizardV(),
  new CharizardVstar(),
  new Cherubi(),
  new Chimchar(),
  new Chimecho(),
  new Cinccino(),
  new Claydol(),
  new Clefable(),
  new Clefairy(),
  new Corphish(),
  new Crawdaunt(),
  new Cubchoo(),
  new Dedenne(),
  new DracovishV(),
  new DrampaV(),
  new Druddigon(),
  new Dusclops(),
  new Dusknoir(),
  new Duskull(),
  new Eiscue(),
  // new Electabuzz(),
  new Electivire(),
  new Empoleon(),
  new EnteiV(),
  new Escavalier(),
  new Exeggcute(),
  new Exeggutor(),
  new Farfetchd(),
  new Floatzel(),
  new Flygon(),
  new FlygonV(),
  new Fraxure(),
  new Gabite(),
  new Garchomp(),
  new Gible(),
  new Golett(),
  new Golurk(),
  new GranbullV(),
  new Grimer(),
  new Grimmsnarl(),
  new Grotle(),
  new Hawlucha(),
  new Haxorus(),
  new Heatran(),
  new Hitmontop(),
  new HonchkrowV(),
  new Impidimp(),
  new Infernape(),
  new KinglerV(),
  new KinglerVmax(),
  new Klang(),
  new Klink(),
  new Klinklang(),
  new Lapras(),
  new Liepard(),
  new Lucario(),
  new LumineonV(),
  new Luxio(),
  new Luxray(),
  new Magmar(),
  new Magmortar(),
  new Manaphy(),
  new Mewtwo(),
  new Milcery(),
  new MimikyuVmax(),
  new Minccino(),
  new Moltres(),
  new Monferno(),
  new Morgrem(),
  new MorpekoV(),
  new Mothim(),
  new Muk(),
  new Nosepass(),
  new Pachirisu(),
  new PikachuV(),
  new Piplup(),
  new Prinplup(),
  new Probopass(),
  new Purrloin(),
  new RaichuV(),
  new RaikouV(),
  new Riolu(),
  new Sawk(),
  new ShayminV(),
  new ShayminVSTAR(),
  new Shinx(),
  new Shroomish(),
  new Sigilyph(),
  new SimisearV(),
  new Sneasel(),
  new Snorunt(),
  new Spiritomb(),
  new Staraptor(),
  new Staravia(),
  new Starly(),
  new Starmie(),
  new Staryu(),
  new Throh(),
  new Torkoal(),
  new Tornadus(),
  new Torterra(),
  new Trapinch(),
  new Tropius(),
  new Turtwig(),
  new Vibrava(),
  new Weavile(),
  new WhimsicottV(),
  new WhimsicottVSTAR(),
  new Wormadam(),
  new Wormadam2(),
  new Wormadam3(),
  new ZarudeV(),

  // Trainers
  new AcerolasPremonition(),
  new Barry(),
  new BlunderPolicy(),
  new CafeMaster(),
  new CherensCare(),
  new CleansingGloves(),
  new CollapsedStadium(),
  new CynthiasAmbition(),
  new FriendsInGalar(),
  new Gloria(),
  new HuntingGloves(),
  new Kindler(),
  new MagmaBasin(),
  new MarniesPride(),
  new PotHelmet(),
  new RoseannesBackup(),
  new TeamYellsCheer(),

  // Energy
  new DoubleTurboEnergy(),

  // Other Prints (Reprints & Alt Arts)
  new UltraBallHR(),
  new WhimsicottVFA(),
  new WhimsicottVSTARRR(),
  new MimikyuVBRS(),
  new ZamazentaVBRS(),
  new BossOrdersBRS(),
  new ChoiceBeltBRS(),
  new ProfessorsResearchBRS(),
  new UltraBallBRS(),
  new ShayminV2BRS(),
  new LumineonV2BRS(),
  new LumineonV3BRS(),
  new RaichuV2BRS(),
  new ZamazentaV2BRS(),
  new ArceusV2BRS(),
  new ArceusV3BRS(),
  new CherensCare2BRS(),
  new CynthiasAmbition2BRS(),
  new Kindler2BRS(),
  new MarniesPride2BRS(),
  new RoseannesBackup2BRS(),
  new ShayminVSTAR2BRS(),
  new ArceusVSTAR2BRS(),
  new CherensCare3BRS(),
  new CynthiasAmbition3BRS(),
  new Kindler3BRS(),
  new RoseannesBackup3BRS(),
  new GalarianArticunoVBRS(),
  new GalarianZapdosVBRS(),
  new GalarianMoltresVBRS(),
  new ArceusVSTAR3BRS(),
  new MagmaBasin2BRS(),
  new GrassEnergyBRS(),
  new FireEnergyBRS(),
  new WaterEnergyBRS(),
  new LightningEnergyBRS(),
  new PsychicEnergyBRS(),
  new FightingEnergyBRS(),
  new DarknessEnergyBRS(),
  new MetalEnergyBRS(),
  new OctilleryBRS(),
  new ZekromBRS(),
  new Dusknoir2BRS(),
  new Alcremie2BRS(),
  new HoundoomBRS(),
  new OranguruBRS(),
  new SylveonVBRS(),
  new SylveonVMAXBRS(),
  new MimikyuV2BRS(),
  new SingleStrikeUrshifuVBRS(),
  new SingleStrikeUrshifuVMAXBRS(),
  new RapidStrikeUrshifuVBRS(),
  new RapidStrikeUrshifuVMAXBRS(),
  new UmbreonVBRS(),
  new UmbreonVMAXBRS(),
  new AcerolasPremonition2BRS(),
  new CafeMaster2BRS(),
  new Gloria2BRS(),
  new SingleStrikeStyleMustardBRS(),
  new SingleStrikeUrshifuVMAX2BRS(),
  new RapidStrikeUrshifuVMAX2BRS(),
  new BurmyBRS(),
  new KarrablastBRS(),
  new FreshWaterSetBRS(),
  new CharizardV2(),
  new CharizardV3(),
  new GranbullV2(),
  new HonchkrowV2(),
  new HonchkrowV3(),
  new FlygonV2(),
  new Barry2(),
  new CharizardVstar2(),
];
