...
const setCodeToTcgDexId: { [key: string]: string } = {
  // Sword & Shield
  SSH: 'swsh1',
  RCL: 'swsh2',
  DAA: 'swsh3',
...
const tcgDexIdToSetCode: { [key: string]: string } = {};
...
function getSetFolder(setAbbr: string): string {
...
function mapCardTypeSymbol(type: string): string {
  if (!type) return 'C';
...
function mapStage(stage: string): string {
  ...
  if (s === 'vstar') return 'Stage.VSTAR';
  if (s === 'v') return 'Stage.BASIC';
  if (s === 'ex') return 'Stage.BASIC';
  if (s === 'gx') return 'Stage.BASIC';
  return 'Stage.BASIC';
}
...
function formatCost(costArr: string[]): string {
...
function sanitizeForCache(obj: any): any {
...
function stripSdkMetadata(data: any): any {
  const { sdk, ...cleaned } = data;
  return cleaned;
}

async function fetchCardFromTcgDex(card: UnknownCard): Promise<FetchResult> {
...
function generateCardClass(card: UnknownCard, tcgdexData: any): { content: string; fileName: string; folder: string } {
  ...
  const className = toPascalCase(tcgdexData.name?.en || tcgdexData.name || card.name);
  const fileName = toKebabCase(tcgdexData.name?.en || tcgdexData.name || card.name) + '.ts';
  const folder = getSetFolder(card.setCode);

  let props = '';
  const imports: string[] = [`import { ${baseClass} } from '${importPath}';`];
  ...
  if (isPokemon) {
    ...
    if (tcgdexData.evolveFrom) {
      props += `  public evolvesFrom = '${escapeSingleQuotes(tcgdexData.evolveFrom?.en || tcgdexData.evolveFrom)}';\n`;
    }

    // Card type
    const cardType = tcgdexData.types?.[0] || tcgdexData.type || 'Colorless';
    ...
    if (tcgdexData.abilities?.length) {
      const powers = tcgdexData.abilities.map((a: any) => {
        const name = escapeSingleQuotes(a.name?.en || a.name || '');
        const text = escapeSingleQuotes(a.effect?.en || a.effect || '').replace(/\n/g, '\\n');
    ...
    if (tcgdexData.attacks?.length) {
      const attacks = tcgdexData.attacks.map((a: any) => {
        const name = escapeSingleQuotes(a.name?.en || a.name || '');
    ...
    if (tcgdexData.effect) {
      props += `\n  public text: string = '${escapeSingleQuotes(tcgdexData.effect?.en || tcgdexData.effect).replace(/\n/g, '\\n')}';\n`;
    }
  } else if (isEnergy) {
  ...
  props += `  public name: string = '${escapeSingleQuotes(tcgdexData.name?.en || tcgdexData.name || card.name)}';\n`;
  props += `  public fullName: string = '${escapeSingleQuotes(tcgdexData.name?.en || tcgdexData.name || card.name)} ${card.setCode.toUpperCase()}';\n`;
...
async function main() {
  ...
  for (const result of fetchResults) {
    if (result.tcgdexData) {
      const cardIndex = unknownCardsData.cards.findIndex(
        c => c.name === result.card.name && c.setCode === result.card.setCode && c.setNumber === result.card.setNumber
      );
      if (cardIndex !== -1) {
        // Add tcgdex data to the card entry (without SDK metadata)
        (unknownCardsData.cards[cardIndex] as any).tcgdexData = stripSdkMetadata(result.tcgdexData);
      }
    }
  }
