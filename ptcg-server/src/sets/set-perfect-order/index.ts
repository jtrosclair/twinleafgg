import { Card } from '../../game/store/card/card';
import { Barbaracle } from './barbaracle';
import { Binacle } from './binacle';
import { CoreMemory } from './core-memory';
import { LumioiseCity } from './lumioise-city';
import { MegaSkarmoryex } from './mega-skarmory-ex';
import { MegaZygardeex } from './mega-zygarde-ex';
import { Meowthex } from './meowth-ex';
import { Naveen } from './naveen';
import { RockFightingEnergy } from './rock-fighting-energy';
import { Tarragon } from './tarragon';
import { Scatterbug } from './scatterbug';
import { Spewpa } from './spewpa';
import { Vivillon } from './vivillon';
import { Rowlet } from './rowlet';
import { Dartrix } from './dartrix';
import { Decidueyeex } from './decidueye-ex';
import { Seel } from './seel';
import { Dewgong } from './dewgong';
import { Staryu } from './staryu';
import { MegaStarmieex } from './mega-starmie-ex';
import { GrowGrassEnergy } from './grow-grass-energy';
import { AntiqueJawFossil } from './antique-jaw-fossil';
import { AntiqueSailFossil } from './antique-sail-fossil';
import { Tyrunt } from './tyrunt';
import { Tyrantrum } from './tyrantrum';
import { Amaura } from './amaura';
import { Aurorus } from './aurorus';
import { Spinarak } from './spinarak';
import { Ariados } from './ariados';
import { Snivy } from './snivy';
import { Servine } from './servine';
import { Serperior } from './serperior';
import { Fletchinder } from './fletchinder';
import { Talonflame } from './talonflame';
import { Salandit } from './salandit';
import { Salazzleex } from './salazzle-ex';
import { Turtonator } from './turtonator';
import { Shinx } from './shinx';
import { Luxio } from './luxio';
import { Luxray } from './luxray';
import { Dedenne } from './dedenne';
import { Honedge } from './honedge';
import { Doublade } from './doublade';
import { Aegislash } from './aegislash';
import { EnergySwatter } from './energy-swatter';
import { LumioseGalette } from './lumiose-galette';
import { RosasEncouragement } from './rosas-encouragement';
import { Jacinthe } from './jacinthe';
import { TelepathPsychicEnergy } from './telepath-psychic-energy';
import { Hawlucha } from './hawlucha';
import { Yveltalex } from './yveltal-ex';
import { Clefairy } from './clefairy';
import { MegaClefableex } from './mega-clefable-ex';
import { Spritzee } from './spritzee';
import { Aromatisse } from './aromatisse';
import { Klefki } from './klefki';
import { Rattata } from './rattata';
import { Raticate } from './raticate';
import { Snorlax } from './snorlax';
import { Bunnelby } from './bunnelby';
import { Diggersby } from './diggersby';
import { Fletchling } from './fletchling';
import { Furfrou } from './furfrou';
import { Shaymin } from './shaymin';
import { Volcanion } from './volcanion';
import { Mawile } from './mawile';
import { Espurr } from './espurr';
import { Meowstic } from './meowstic';
import { Nosepass } from './nosepass';
import { Probopass } from './probopass';
import { Hippopotas } from './hippopotas';
import { Hippowdon } from './hippowdon';
import { Landorus } from './landorus';
import { Gastly } from './gastly';
import { Haunter } from './haunter';
import { Gengar } from './gengar';
import { Skorupi } from './skorupi';
import { Drapion } from './drapion';
import { ChienPao } from './chien-pao';
import { Laprasex } from './lapras-ex';
import { HoleDiggingShovel } from './hole-digging-shovel';
import { SpewpaIR, RowletIR, TalonflameIR, AurorusIR, DedenneIR, ClefairyIR, EspurrIR, ProbopassIR, DrapionIR, DoubladeIR, RaticateIR, DecidueyeexFA, SalazzleexFA, MegaStarmieexFA, MegaClefableexFA, MegaZygardeexFA, YveltalexFA, MegaSkarmoryexFA, MeowthexFA, EnergyRecyclerFA, SacredAshFA, PokePadFA, PokePadNormal, WondrousPatchFA, TarragonFA, NaveenFA, RosasEncouragementFA, JacintheFA, ForestOfVitalityFA, LumioiseCityFA, JacintheSIR, MegaClefableexSIR, MegaStarmieexSIR, MegaZygardeexHR, MegaZygardeexSIR, MeowthexSIR, RosasEncouragementSIR, JudgePOR } from './other-prints';

export const setPerfectOrder: Card[] = [
  new Barbaracle(),
  new Binacle,
  new CoreMemory(),
  new LumioiseCity(),
  new MegaSkarmoryex(),
  new MegaZygardeex(),
  new Meowthex(),
  new Naveen(),
  new RockFightingEnergy(),
  new Tarragon(),
  new Scatterbug(),
  new Spewpa(),
  new Vivillon(),
  new Rowlet(),
  new Dartrix(),
  new Decidueyeex(),
  new Seel(),
  new Dewgong(),
  new Staryu(),
  new MegaStarmieex(),
  new GrowGrassEnergy(),
  new AntiqueJawFossil(),
  new AntiqueSailFossil(),
  new Tyrunt(),
  new Tyrantrum(),
  new Amaura(),
  new Aurorus(),
  new Spinarak(),
  new Ariados(),
  new Snivy(),
  new Servine(),
  new Serperior(),
  new Fletchinder(),
  new Talonflame(),
  new Salandit(),
  new Salazzleex(),
  new Turtonator(),
  new Shinx(),
  new Luxio(),
  new Luxray(),
  new Dedenne(),
  new Honedge(),
  new Doublade(),
  new Aegislash(),
  new EnergySwatter(),
  new LumioseGalette(),
  new RosasEncouragement(),
  new Jacinthe(),
  new TelepathPsychicEnergy(),
  new Hawlucha(),
  new Yveltalex(),
  new Clefairy(),
  new MegaClefableex(),
  new Spritzee(),
  new Aromatisse(),
  new Klefki(),
  new Rattata(),
  new Raticate(),
  new Snorlax(),
  new Bunnelby(),
  new Diggersby(),
  new Fletchling(),
  new Furfrou(),
  new Shaymin(),
  new Volcanion(),
  new Mawile(),
  new Espurr(),
  new Meowstic(),
  new Nosepass(),
  new Probopass(),
  new Hippopotas(),
  new Hippowdon(),
  new Landorus(),
  new Gastly(),
  new Haunter(),
  new Gengar(),
  new Skorupi(),
  new Drapion(),
  new ChienPao(),
  new Laprasex(),
  new HoleDiggingShovel(),

  // IRs, FAs, SIRs, HR
  new SpewpaIR(),
  new RowletIR(),
  new TalonflameIR(),
  new AurorusIR(),
  new DedenneIR(),
  new ClefairyIR(),
  new EspurrIR(),
  new ProbopassIR(),
  new DrapionIR(),
  new DoubladeIR(),
  new RaticateIR(),
  new DecidueyeexFA(),
  new SalazzleexFA(),
  new MegaStarmieexFA(),
  new MegaClefableexFA(),
  new MegaZygardeexFA(),
  new YveltalexFA(),
  new MegaSkarmoryexFA(),
  new MeowthexFA(),
  new EnergyRecyclerFA(),
  new SacredAshFA(),
  new PokePadNormal(),
  new PokePadFA(),
  new WondrousPatchFA(),
  new TarragonFA(),
  new NaveenFA(),
  new RosasEncouragementFA(),
  new JacintheFA(),
  new ForestOfVitalityFA(),
  new LumioiseCityFA(),
  new MegaStarmieexSIR(),
  new MegaClefableexSIR(),
  new MegaZygardeexSIR(),
  new MeowthexSIR(),
  new RosasEncouragementSIR(),
  new JacintheSIR(),
  new MegaZygardeexHR(),
  new JudgePOR()
];
